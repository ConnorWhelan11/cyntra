...
use ai_utility::{UtilityOption, UtilityPolicy, UtilityPolicyConfig};
...
#[test]
fn utility_policy_tie_breaks_by_option_order() {
    let agent = 1u64;
    let options = vec![
        UtilityOption::new(
            ActionKey("a"),
    ...
    let policy = Box::new(UtilityPolicy::new(options).with_config(UtilityPolicyConfig {
        min_score: 0.0,
    }));
...
#[test]
fn utility_policy_switching_options_cancels_previous_action() {
    let agent = 1u64;
    let options = vec![
        UtilityOption::new(
            ActionKey("a"),
    ...
    let policy = Box::new(UtilityPolicy::new(options));
    let mut brain = Brain::new(agent, policy);
    let mut world = World::default();
