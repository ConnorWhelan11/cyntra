use ai_core::{
    Action, ActionFactory, ActionKey, ActionOutcome, ActionRuntime, ActionStatus, BbKey, Brain,
    PlanExecutorAction, PlanSpec, Policy, TickContext, WorldMut, WorldView,
};
...
enum TestSpec {
    Log(&'static str),
    Wait { name: &'static str, ticks: u32 },
}

#[derive(Debug, Clone, Copy)]
struct TestFactory;

impl ActionFactory<PlanWorld> for TestFactory {
    type Spec = TestSpec;
    ...
    ) -> Box<dyn Action<PlanWorld>> {
        match spec {
            TestSpec::Log(name) => Box::new(InstantLogAction(name)),
            TestSpec::Wait { name, ticks } => Box::new(WaitTicksAction::new(name, *ticks)),
        }
    }
}
...
impl Policy<PlanWorld> for StartPlanOnce {
    fn tick(
        ...
        let plan = PlanSpec::new(vec![
            TestSpec::Log("a"),
            TestSpec::Wait { name: "b", ticks: 3 },
            TestSpec::Log("c"),
        ]);

        actions.ensure_current(
            ActionKey("plan"),
            |_ctx, _agent, _world, _bb| Box::new(PlanExecutorAction::new(plan, TestFactory)),
            ctx,
            agent,
            world,
            blackboard,
...
impl Policy<PlanWorld> for ConditionalPlanPolicy {
    fn tick(
        ...
        let plan = PlanSpec::new(vec![TestSpec::Wait {
            name: "wait",
            ticks: 999,
        }]);
...
#[test]
fn spec_plan_executes_to_completion_without_policy_ticks() {
    let agent = 1u64;
...
#[test]
fn spec_plan_cancel_propagates_to_current_step() {
    let agent = 1u64;
