    ...
    *,
    ...
    for index, case in enumerate(_bench_cases(bench), start=1):
        ...
        issue: dict[str, Any] = {
            ...
            "dk_apply_patch": bool(
                case.get("dk_apply_patch") if "dk_apply_patch" in case else apply_patch
            ),
        }
...
def write_bench_report(
    *,
    ...
    for archive in sorted([p for p in archives_dir.iterdir() if p.is_dir()]):
        ...
        issue_id = (
            str((manifest.get("issue") or {}).get("id")) or str(proof.get("issue_id")) or "unknown"
        )
    ...
    report = {
        ...
        "sampling": _sanitize_sampling(sampling)
        if sampling
        else {"temperature": None, "top_p": None},
        "summary": {
