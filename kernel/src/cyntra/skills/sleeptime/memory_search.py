...
def _search_blocks(
    memory_path: Path, query: str, filters: dict[str, Any] | None, limit: int
) -> list[dict[str, Any]]:
    ...
    for block_type_dir in blocks_dir.iterdir():
        ...
        if filters and "block_type" in filters and block_type_dir.name != filters["block_type"]:
            continue
        ...
        for block_file in block_type_dir.glob("*.json"):
            try:
                ...
                # Apply filters
                if filters:
                    if "domain" in filters and block.get("domain") not in (
                        filters["domain"],
                        "all",
                    ):
                        continue
