...
def tokenize_planner_input(
    ...
    *,
    ...
    tags: list[str] = (
        [t for t in tags_value if isinstance(t, str) and t] if isinstance(tags_value, list) else []
    )
    if tags:
    ...
    runs_list: list[dict[str, Any]] = (
        [r for r in runs if isinstance(r, dict)] if isinstance(runs, list) else []
    )
    ...
    for idx, run in enumerate(runs_list):
        ...
        fail_list = (
            [c for c in fail_codes if isinstance(c, str) and c]
