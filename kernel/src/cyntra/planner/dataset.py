...
def _label_from_run_summary(
    run_summary: dict[str, Any], *, action_space: ActionSpace
) -> dict[str, Any]:
...
def build_planner_input(
    *,
    ...
    query = SimilarRunsQuery(
        ...
        objective_id=str(run_summary.get("objective_id"))
        if run_summary.get("objective_id")
        else None,
    )
...
def build_dataset_examples(
    *,
    ...
    swarm_ids = (
        load_universe_swarm_ids(repo_root, universe_id)
        if universe_id
        else ["serial_handoff", "speculate_vote"]
    )
    action_space = action_space_for_swarms(swarm_ids)
    universe_defaults = (
        load_universe_defaults(repo_root, universe_id)
        if universe_id
        else {"swarm_id": None, "objective_id": None}
    )
