...
class ScaffoldRegistry:
    ...
    def get(
        ...
        if version:
            ...
            # Find latest version
            matching = [k for k in self._scaffolds[category] if k.startswith(f"{scaffold_id}:")]
            if matching:
    ...
    def list_scaffolds(self, category: str | None = None) -> list[dict[str, Any]]:
        """
    ...
    def get_manifest(self, scaffold_id: str, version: str) -> dict[str, Any] | None:
    ...
    def export_manifest(self, output_path: Path) -> None:
        ...
        logger.info(f"Exported {len(self._manifests)} scaffold manifests to {output_path}")


# Global registry instance
_global_registry: ScaffoldRegistry | None = None
...
def list_scaffolds(category: str | None = None) -> list[dict[str, Any]]:
    """
    List available scaffolds.
