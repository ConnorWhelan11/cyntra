...
class SverchokConfig:
    ...
    node_tree_cache_dir: Path | None = None
...
class SverchokScaffold(ScaffoldBase):
    ...
    def __init__(self, config: SverchokConfig | None = None):
        super().__init__()
        self.config = config or SverchokConfig()
        self._node_tree: SverchokNodeTree | None = None
    ...
    def get_node_tree(self) -> SverchokNodeTree | None:
    ...
    def generate_blender_script(
        ...
        if available and self.config.enabled:
            ...
            logger.info(f"Sverchok not available ({message}), using Geometry Nodes fallback")
