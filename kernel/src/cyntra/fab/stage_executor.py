...
class StageExecutor:
    ...
    def execute_blender_stage(
        ...
        try:
            ...
            print("Running Blender with stage script...")
...
def _fab_subprocess_env(*, repo_root: Path | None) -> dict[str, str]:
    ...
    kernel_src = (repo_root / "kernel" / "src").resolve()
    if kernel_src.exists():
        existing = env.get("PYTHONPATH")
        env["PYTHONPATH"] = (
            str(kernel_src) if not existing else str(kernel_src) + os.pathsep + existing
        )
    return env
...
def execute_gate_stage(
    ...
    for gate_ref in gate_refs:
        ...
        if gate_path is None:
            ...
            metadata["gates"].append(
                {"gate": gate_name, "passed": False, "error": "config not found"}
            )
            continue
        ...
        if is_godot_gate:
            ...
            cmd = [
                ...
                "--out",
                str(gate_out),
                *(["--render-dir", str(render_dir)] if render_dir is not None else []),
                "--json",
            ]
        ...
        if not passed:
            ...
            if reason is None and gate_json and isinstance(gate_json, dict):
                reason = (
                    str(gate_json.get("failures") or gate_json.get("errors") or "").strip() or None
                )
    ...
    try:
        ...
        next_actions.sort(
            key=lambda item: (int(item.get("priority") or 99), str(item.get("action") or ""))
        )
        ...
        verdict_payload: dict[str, Any] = {
            ...
            + ", ".join(
                [e.get("gate") for e in gate_entries if isinstance(e, dict) and not e.get("passed")]
            )
            if gate_entries
...
def execute_comfyui_stage(
    ...
    seed = int(determinism.get("seed", 42)) if isinstance(determinism, dict) else 42
    ...
    async def _run_workflow() -> dict[str, Any]:
        ...
        try:
            ...
            for _node_id, paths in downloaded.items():
                for path in paths:
                    outputs.append(str(path))
            metadata["downloaded_files"] = {
                node_id: [str(p) for p in paths] for node_id, paths in downloaded.items()
            }
        except Exception as e:
