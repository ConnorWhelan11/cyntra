...
class IterationManager:
    ...
    def should_retry(self, issue_id: str) -> bool:
        ...
        if state.status == "abandoned":
            return False
        return not state.current_iteration >= state.max_iterations
    ...
    def should_escalate(self, issue_id: str) -> bool:
        ...
        if len(state.history) >= 2:
            ...
            for entry in state.history[-2:]:
                hard_fails = [
                    c for c in entry.get("fail_codes", []) if c in self.gate_config.hard_fail_codes
                ]
    ...
    def _build_repair_description(
        ...
        parts = [
            ...
            "The previous attempt failed gate evaluation with the following issues:\n",
...
def should_create_repair_issue(
    ...
    return not iteration_index >= max_iterations - 1
