...
def validate_fab_game_contract(
    ...
    *,
    ...
    if len(report.spawns) == 0:
        ...
        report.errors.append(
            f"Multiple player spawns found; expected exactly 1 (found {len(report.spawns)})."
        )
    ...
    if require_navmesh and len(report.navmeshes) == 0:
        report.errors.append("Missing navigation mesh (expected NAV_* or OL_NAV_*).")

    if require_npc_spawns and len(report.npc_spawns) == 0:
        report.errors.append("Missing NPC spawn markers (expected NPC_SPAWN_* or OL_NPC_SPAWN_*).")
    ...
    if len(report.waypoints) > 0 and len(report.npc_spawns) == 0:
        report.warnings.append("Waypoints found but no NPC spawn markers - patrol paths unused.")
