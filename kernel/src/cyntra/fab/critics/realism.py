...
class RealismCritic:
    ...
    def _detect_missing_textures(self, image: Any) -> float:
        ...
        try:
            ...
            is_magenta = (r > self.MAGENTA_THRESHOLD) & (g < 0.4) & (b > self.MAGENTA_THRESHOLD)
    ...
    def evaluate(
        ...
        if views_evaluated > 0:
            mean_aesthetic = sum(v.aesthetic_score for v in view_scores) / views_evaluated
        ...
        if view_scores:
            aggregate_artifacts = {
                "mean_noise": sum(v.artifact_metrics.noise_estimate for v in view_scores)
                / len(view_scores),
                "max_noise": max(v.artifact_metrics.noise_estimate for v in view_scores),
                ...
                "mean_edge_density": sum(v.artifact_metrics.edge_density for v in view_scores)
                / len(view_scores),
            }
        ...
        score = 0.5 * mean_aesthetic + 0.5 * mean_quality if views_evaluated > 0 else 0.0
