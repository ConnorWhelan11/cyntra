...
Usage:
    python -m cyntra.fab.critics.cli category --beauty-dir /path/to/beauty --clay-dir /path/to/clay
    python -m cyntra.fab.critics.cli geometry --mesh /path/to/asset.glb
"""
...
def run_geometry(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        if result.bounds:
            print(
                f"Bounds:          {result.bounds.length:.2f}L x {result.bounds.width:.2f}W x {result.bounds.height:.2f}H m"
            )
...
def main(args: list = None) -> int:
    ...
    cat_parser.add_argument(
        "--beauty-dir", type=str, required=True, help="Directory with beauty renders"
    )
