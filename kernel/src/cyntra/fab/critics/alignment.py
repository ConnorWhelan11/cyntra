...
class AlignmentCritic:
    ...
    def _encode_image(self, image_path: Path) -> Any | None:
    ...
    def _encode_texts(self, texts: list[str]) -> Any | None:
    ...
    def _compute_similarity(self, image_features: Any, text_features: Any) -> list[float]:
    ...
    def _extract_attributes_from_prompt(self, prompt: str) -> dict[str, str]:
        ...
        # Check for colors
        for color in self.COLOR_PROMPTS:
            if color in prompt_lower:
                attributes["color"] = color
                break

        # Check for styles
        for style in self.STYLE_PROMPTS:
            if style in prompt_lower:
                attributes["style"] = style
                break

        # Check for body types
        for body_type in self.BODY_TYPE_PROMPTS:
    ...
    def evaluate(
        ...
        for render_path in render_paths:
            ...
            prompt_sim, margin, attr_scores = self._evaluate_view(render_path, prompt, decoy_list)
        ...
        if views_evaluated > 0:
            mean_similarity = sum(v.prompt_similarity for v in view_scores) / views_evaluated
