...
class TestCodeReviewerHandler:
    ...
    def test_no_changes_returns_approve(self, sample_context: HookContext) -> None:
        ...
        with patch("cyntra.hooks.code_reviewer.get_git_diff", return_value=""):
...
+console.log("test")
 def main():
     pass
"""
        with patch("cyntra.hooks.code_reviewer.get_git_diff", return_value=diff_with_debug):
            result = code_reviewer_handler(sample_context)
...
+password = "secret123"
 def main():
     pass
"""
        with patch("cyntra.hooks.code_reviewer.get_git_diff", return_value=diff):
            result = code_reviewer_handler(sample_context)
