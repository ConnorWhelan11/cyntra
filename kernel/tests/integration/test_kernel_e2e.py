"""End-to-end integration tests for Cyntra."""
...
class TestScheduling:
    ...
    def test_respects_token_limit(self, mock_beads_repo: Path) -> None:
        ...
        total_tokens = sum(i.dk_estimated_tokens or 50000 for i in result.scheduled_lanes)
...
class TestDryRun:
    ...
    def test_dry_run_makes_no_changes(self, mock_beads_repo: Path) -> None:
        ...
        config_path = mock_beads_repo / ".cyntra" / "config.yaml"
...
class TestFullCycle:
    ...
    async def test_logs_events(
        ...
        """Should log events to .cyntra/logs/events.jsonl."""
        config_path = mock_beads_repo / ".cyntra" / "config.yaml"
        ...
        events_file = mock_beads_repo / ".cyntra" / "logs" / "events.jsonl"
...
class TestSpeculateMode:
    ...
    def test_high_risk_triggers_speculate(self, mock_beads_repo: Path) -> None:
        ...
        assert all(isinstance(issue_id, str) for issue_id in spec_ids)
...
class TestStateUpdates:
    ...
    def test_add_event(self, mock_beads_repo: Path) -> None:
        ...
        events_file = config.logs_dir / "events.jsonl"
