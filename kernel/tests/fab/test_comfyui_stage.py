...
class TestResolveWorkflowPath:
    ...
    def test_absolute_path(
        self, sample_workflow: Path, mock_world_config: MagicMock, temp_dirs: dict[str, Path]
    ):
    ...
    def test_in_comfyui_workflows_dir(
        self, sample_workflow: Path, temp_dirs: dict[str, Path], mock_world_config: MagicMock
    ):
...
class TestExecuteComfyuiStage:
    ...
    def test_successful_execution(
        ...
        mock_client.download_outputs = AsyncMock(
            return_value={
                "2": [output_file],
            }
        )
...
class TestConnectionErrors:
    ...
    def test_connection_error(
        ...
        mock_client.health_check = AsyncMock(
            side_effect=ComfyUIConnectionError("Connection refused")
        )
        mock_client_cls.return_value = mock_client
        mock_client_cls.load_workflow = MagicMock(return_value={})
        mock_client_cls.inject_seed = MagicMock(return_value={})
...
class TestWorkflowLoadingErrors:
    ...
    def test_invalid_workflow_json(
        ...
        mock_client.health_check = AsyncMock(return_value=True)
        mock_client_cls.return_value = mock_client
        mock_client_cls.load_workflow = MagicMock(
