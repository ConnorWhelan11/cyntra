...
Cyntra is configured via a YAML file at `.cyntra/config.yaml`. This guide covers all configuration options.
...
cyntra --config ./my-config.yaml run
```
...
observability:
  log_level: "info"
  event_log_dir: ".cyntra/logs"
  archive_dir: ".cyntra/archives"
...
Create `.cyntra/config.local.yaml` for local overrides (git-ignored):
