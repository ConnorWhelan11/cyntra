...
def _subprocess_env() -> dict[str, str]:
    env = dict(os.environ)
    src_root = str(REPO_ROOT / "cyntra-kernel" / "src")
...
class TestFabIntegration:
    ...
    def test_geometry_critic_evaluates_asset(self, test_asset, temp_dir):
        ...
        if importlib.util.find_spec("trimesh") is None:
            pytest.skip("trimesh not installed (cyntra-kernel[fab] extra required)")
    ...
    def test_schema_validation(self):
        ...
        schemas_dir = REPO_ROOT / "cyntra-kernel" / "schemas" / "fab"
