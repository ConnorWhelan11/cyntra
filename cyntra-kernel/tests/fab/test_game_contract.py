...
# Add the outora-library to path for testing
outora_lib_path = Path(__file__).parent.parent.parent.parent / "fab" / "outora-library" / "src"
if str(outora_lib_path) not in sys.path:
    sys.path.insert(0, str(outora_lib_path))

from outora_library.game_contract import (
    FabRole,
...
class TestFabRole:
    ...
    def test_all_roles_defined(self):
        ...
        expected = [
            "SPAWN_PLAYER", "COLLIDER", "TRIGGER", "INTERACT",
            "NAVMESH", "NPC_SPAWN", "ITEM_SPAWN", "AUDIO_ZONE", "WAYPOINT"
        ]
...
class TestFabGameContractReport:
    ...
    def test_npc_ready(self):
        ...
        # Both present
        report4 = FabGameContractReport(
            navmeshes=["NAV_floor"],
            npc_spawns=["NPC_SPAWN_librarian"]
        )
...
class TestExtractIdFromMarker:
    ...
    def test_interact_extraction(self):
        ...
        assert extract_id_from_marker("INTERACT_bookshelf_ancient", "INTERACT_") == "bookshelf_ancient"
...
class TestValidateGameplayAgainstMarkers:
    ...
    def test_orphaned_markers_strict_mode(self, sample_gameplay_config):
        ...
        report = validate_gameplay_against_markers(
            sample_gameplay_config, markers, strict=True
        )
        assert report.valid is False
