...
def test_universe_patterns_store_emits_fab_fail_code_antipatterns(tmp_path: Path) -> None:
    ...
    for idx, started_ms in enumerate((1000, 2000), start=1):
        ...
        write_run_context(run_dir, RunContext(universe_id="medica", world_id="w1", objective_id="obj"))
        (run_dir / "run_meta.json").write_text(json.dumps({"started_ms": started_ms}), encoding="utf-8")
