...
class TestHookRunner:
    ...
    def test_replace_existing_hook(self) -> None:
        ...
        hook1 = HookDefinition(
            name="my-hook",
            trigger=HookTrigger.POST_EXECUTION,
            handler=lambda ctx: HookResult(hook_name="my-hook", success=True, output={"version": 1}),
        )
        hook2 = HookDefinition(
            name="my-hook",
            trigger=HookTrigger.POST_EXECUTION,
            handler=lambda ctx: HookResult(hook_name="my-hook", success=True, output={"version": 2}),
        )
