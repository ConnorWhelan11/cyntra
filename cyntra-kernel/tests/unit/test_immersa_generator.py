...
def test_scan_glb_assets_finds_outora_and_run_artifacts(tmp_path: Path) -> None:
    outora_dir = tmp_path / "fab" / "outora-library"
    ...
    assert "fab/outora-library/car.glb" in rel_paths
    assert ".cyntra/runs/run_1/artifacts/artifact.glb" in rel_paths
    assert ".cyntra/runs/run_1/other/ignored.glb" not in rel_paths

    outora_asset = next(a for a in assets if a.rel_path == "fab/outora-library/car.glb")
    assert outora_asset.url == "/artifacts/fab/outora-library/car.glb"


def test_generate_deck_uses_stable_ids_and_expected_shape() -> None:
    rel_path = "fab/outora-library/car.glb"
    asset = ImmersaAsset(
        name="car.glb",
        rel_path=rel_path,
