...
from cyntra.kernel.config import KernelConfig, ToolchainConfig, GatesConfig
...
class TestDispatcher:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_async_respects_timeout_override(
        ...
        class StubAdapter:
            ...
            def __init__(self) -> None:
                self.last_timeout: timedelta | None = None

            async def execute(self, manifest: dict, workcell_path: Path, timeout: timedelta) -> PatchProof:
                self.last_timeout = timeout
                return PatchProof(
                    schema_version="1.0.0",
                    workcell_id=str(manifest.get("workcell_id") or "wc-test"),
                    issue_id=str((manifest.get("issue") or {}).get("id") or "unknown"),
                    status="failed",
                    patch={"files_modified": [], "diff_stats": {"files_changed": 0, "insertions": 0, "deletions": 0}},
                    verification={"gates": {}, "all_passed": False, "blocking_failures": []},
                    metadata={"toolchain": "stub", "duration_ms": 1},
                    confidence=0.1,
        ...
        dispatcher = Dispatcher(config)
        stub = StubAdapter()
        dispatcher._adapters["codex"] = stub  # type: ignore[attr-defined]

        issue = make_issue(tool_hint="codex")
        ...
        await dispatcher.dispatch_async(
            issue,
            workcell_path,
            toolchain_override="codex",
            manifest_overrides={"planner": {"timeout_seconds_override": 60}},
        )

        assert stub.last_timeout == timedelta(seconds=60)
