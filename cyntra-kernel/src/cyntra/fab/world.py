...
def cmd_validate(args):
    ...
    if args.output:
        ...
        run_dir = inferred_run or (
            Path.cwd() / ".cyntra" / "runs" / f"validate_{int(time.time())}"
        )
...
def cmd_prune(args):
    """Prune large intermediate artifacts from a run directory (disk saver)."""
    import json
    import shutil
    ...
    keep_stages = set(args.keep_stage or [])

    # Prune baked blend files (these are typically huge and not required for shipping/viewing).
    ...
    if world_dir.exists():
        for candidate in sorted(world_dir.glob("*_baked.blend*")):
            ...
            removed.append(
                {
                    "kind": "file",
                    "path": str(candidate.relative_to(run_dir)) if candidate.is_absolute() else str(candidate),
            ...
            if not args.dry_run:
                try:
                    ...
                    print(f"⚠ Failed to delete {candidate}: {exc}")

    # Prune stage directories (keep gate stage by default).
    stages_dir = run_dir / "stages"
    if stages_dir.exists():
        for child in sorted(stages_dir.iterdir()):
            if not child.is_dir():
                continue
            if child.name in keep_stages:
                continue
            removed.append(
                {
                    "kind": "dir",
                    "path": str(child.relative_to(run_dir)) if child.is_absolute() else str(child),
                }
            )
            if not args.dry_run:
                try:
    ...
    out_path = run_dir / "pruned_intermediates.json"
    try:
        out_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    except Exception as exc:
        print(f"⚠ Failed to write prune report {out_path}: {exc}")

    print(f"✓ Pruned {len(removed)} items (report: {out_path})")
    return 0
...
def main():
    ...
    publish_parser.add_argument(
        "--run",
        required=True,
        help="Path to run directory (e.g., .cyntra/runs/run_001)",
    )
    publish_parser.add_argument(
        "--viewer",
    ...
    prune_parser = subparsers.add_parser(
        "prune",
        help="Prune large intermediate artifacts from a run directory",
    )
    ...
    prune_parser.add_argument(
        ...
        action="append",
        default=["validate"],
        help="Stage directory name(s) to keep (repeatable, default: validate)",
    )
    prune_parser.add_argument(
        "--dry-run",
        action="store_true",
