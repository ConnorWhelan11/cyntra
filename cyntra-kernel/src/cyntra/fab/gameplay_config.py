...
class EntityConfig:
    ...
    entity_type: Optional[str] = None  # npc, key_item, consumable, etc.
    behavior: Optional[str] = None
    patrol_path: Optional[str] = None
...
class GameplayConfig:
    ...
    def _check_objective_cycles(self):
        ...
        for obj_id in graph:
            if obj_id not in visited:
                if has_cycle(obj_id):
                    raise ValueError(
                        f"Circular dependency detected in objectives involving '{obj_id}'"
                    )
    ...
    def get_entity(self, entity_id: str) -> Optional[EntityConfig]:
        """Get entity configuration by ID."""
        return self.entities.get(entity_id)

    def get_interaction(self, interaction_id: str) -> Optional[InteractionConfig]:
    ...
    def get_audio_zone(self, zone_id: str) -> Optional[AudioZoneConfig]:
        """Get audio zone configuration by ID."""
        return self.audio_zones.get(zone_id)

    def get_objective(self, objective_id: str) -> Optional[ObjectiveConfig]:
...
def load_gameplay_config(gameplay_path: Path) -> GameplayConfig:
    ...
    """
    if gameplay_path.is_dir():
        config_file = gameplay_path / "gameplay.yaml"
    else:
        config_file = gameplay_path
