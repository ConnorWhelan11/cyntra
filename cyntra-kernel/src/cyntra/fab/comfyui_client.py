...
class ComfyUIClient:
    ...
    async def wait_for_completion(
        ...
        while True:
            ...
            try:
                ...
                is_running = any(
                    item[1] == prompt_id for item in running if isinstance(item, list)
                )
                is_pending = any(
                    item[1] == prompt_id for item in pending if isinstance(item, list)
                )
    ...
    def _parse_outputs(self, outputs: dict[str, Any]) -> dict[str, list[Path]]:
        ...
        for node_id, node_outputs in outputs.items():
            ...
            # Handle other file outputs (gifs, videos, etc.)
    ...
    def inject_params(
        ...
        for node_id, node in modified.items():
            ...
            if class_type == "CLIPTextEncode":
                if "positive" in title or "positive" in node_id_lower:
                    if "positive_prompt" in params:
                        inputs["text"] = params["positive_prompt"]
                elif "negative" in title or "negative" in node_id_lower:
