...
def universe_status(ctx: click.Context, universe_id: str) -> None:
    ...
    header.add_row("policies.determinism", json.dumps(policies.get("determinism") or {}, sort_keys=True))
...
@click.argument("universe_id", type=str)
@click.option("--run-id", "run_id", type=str, required=True, help="Run directory name under `.cyntra/runs/`")
@click.option("--output", type=Path, help="Override output runs.jsonl path")
@click.pass_context
def universe_index_update(ctx: click.Context, universe_id: str, run_id: str, output: Path | None) -> None:
...
@click.pass_context
def universe_index_rebuild_generations(ctx: click.Context, universe_id: str, output: Path | None) -> None:
...
@click.option("--max-evidence", type=int, default=5, show_default=True, help="Max evidence_runs per pattern")
@click.pass_context
...
def universe_frontiers_rebuild(
    ...
    console.print(f"[green]✓[/green] Wrote {len(paths)} world frontier file(s) ({total_points} point(s)) to {output_path}")
...
@click.option("--output", type=Path, help="Run output dir (default: .cyntra/runs/world_<id>_seed<seed>_<ts>)")
...
@click.option("--prune-intermediates", is_flag=True, help="Delete intermediate stage dirs after use")
@click.pass_context
def world_build(
    ctx: click.Context,
    universe: str | None,
    ...
    if universe:
        ...
        if world is not None:
            ...
            resolved_world_path = (repo_root / world_ref).resolve() if not Path(world_ref).is_absolute() else Path(world_ref).resolve()
    ...
    if run_dir is None:
        ...
        run_dir = config.kernel_dir / "runs" / f"world_{resolved_world_id}_seed{resolved_seed}_{timestamp}"
...
@click.option("--action-low", type=float, default=0.1, show_default=True, help="Low action threshold")
@click.option("--delta-v-low", type=float, default=0.05, show_default=True, help="Low delta-V threshold")
...
def evolve(ctx: click.Context) -> None:
    ...
    parser.add_argument("--objective-id", "--objective", dest="objective_id", help="Objective id (defaults from universe)")
    parser.add_argument("--swarm-id", "--swarm", dest="swarm_id", help="Swarm id (defaults from universe)")
    parser.add_argument("--generations", type=int, default=3)
    parser.add_argument("--population", dest="population_size", type=int, help="Population size override")
    ...
    try:
        universe_cfg = load_universe(args.universe, repo_root=config.repo_root, validate_worlds=False)
    except UniverseLoadError as exc:
        raise click.ClickException(str(exc)) from exc
    ...
    if not resolved_objective:
        raise click.ClickException("No objective id specified and universe has no defaults.objective_id")
    ...
    try:
        ...
        build_generations_index(
            ...
            runs_dir=config.kernel_dir / "runs",
            output_path=config.kernel_dir / "universes" / universe_cfg.universe_id / "index" / "generations.jsonl",
        )
    except Exception:
        pass
    ...
    try:
        ...
        runs_dir = config.kernel_dir / "runs"
        runs_index = config.kernel_dir / "universes" / universe_cfg.universe_id / "index" / "runs.jsonl"
        update_runs_index(
            universe_id=universe_cfg.universe_id,
            runs_dir=runs_dir,
...
@click.option(
    ...
    default=True,
    show_default=True,
    help="Include `fab/outora-library/**/*.glb`",
)
@click.option(
...
@click.option("--output", type=Path, help="Output directory (default: .cyntra/benches/<name>_<timestamp>)")
...
@click.option("--n-similar", type=int, default=8, show_default=True, help="Similar runs per example")
@click.option("--include-world/--no-include-world", default=True, help="Include fab-world runs")
@click.pass_context
...
@click.option("--dropout", type=float, default=0.1, show_default=True)
@click.option("--tbptt-len", type=int, default=64, show_default=True, help="Truncated BPTT chunk length")
@click.option("--epochs", type=int, default=10, show_default=True)
@click.option("--batch-size", type=int, default=32, show_default=True)
@click.option("--lr", type=float, default=3e-4, show_default=True)
@click.option("--seed", type=int, default=42, show_default=True)
...
def planner_infer(
    ...
    from cyntra.planner.artifacts import build_planner_input_v1, collect_history_candidates, system_state_snapshot
    from cyntra.planner.dataset import infer_default_universe_id, load_universe_defaults, load_universe_swarm_ids
    ...
    system_state = system_state_snapshot(
        ...
        available_toolchains=dispatcher.get_available_toolchains() or list(config.toolchain_priority),
    )
...
@click.option("--toolchain", "toolchain_override", type=str, default=None, help="Force toolchain (debug)")
@click.pass_context
