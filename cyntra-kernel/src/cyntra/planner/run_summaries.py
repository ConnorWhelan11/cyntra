...
def build_archive_run_summary(workcell_archive_dir: Path) -> dict[str, Any] | None:
    ...
    result: dict[str, Any] = {
        "run_id": str(proof.get("workcell_id") or manifest.get("workcell_id") or workcell_archive_dir.name),
...
def _build_run_summary_from_rollout(
    ...
    verification = outcomes.get("verification") if isinstance(outcomes.get("verification"), dict) else {}
    all_passed = bool(verification.get("all_passed", False))
