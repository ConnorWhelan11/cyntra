...
class OnnxPlanner:
    def __init__(self, bundle_dir: Path) -> None:
        ...
        if not isinstance(swarm_ids_raw, list) or not all(isinstance(s, str) for s in swarm_ids_raw):
        ...
        seq_len = 512
        if input_ids_info and isinstance(input_ids_info.shape, list) and len(input_ids_info.shape) == 2:
    ...
    def _predict_probs(
        ...
        input_ids, attention_mask = encode_tokens(tokens, vocab=self.cfg.vocab, seq_len=self.cfg.seq_len)
        ...
        *,
        ...
        for swarm_id, max_candidates, max_minutes, max_iterations in valid_actions(
            ...
            if score > best_score:
                ...
                if json.dumps((swarm_id, max_candidates, max_minutes, max_iterations), sort_keys=True) < json.dumps(
                    best, sort_keys=True
                ):
                    best = (swarm_id, max_candidates, max_minutes, max_iterations)
        ...
        if best is None:
            ...
            best = (
                ...
                self.cfg.max_candidates_bins[min(candidates_idx, len(self.cfg.max_candidates_bins) - 1)],
                self.cfg.max_minutes_bins[min(minutes_idx, len(self.cfg.max_minutes_bins) - 1)],
                self.cfg.max_iterations_bins[min(iterations_idx, len(self.cfg.max_iterations_bins) - 1)],
            )
