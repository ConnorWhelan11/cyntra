...
def _schemas_dir() -> Path:
    # cyntra-kernel/src/cyntra/universe/evolve_world.py -> cyntra-kernel/
...
def load_world_genome_surface(*, world_yaml: Path) -> WorldGenomeSurface:
    ...
    if genome_path.exists():
        ...
        world_config_id_s = str(world_config_id).strip() if isinstance(world_config_id, str) else None
    ...
    if not derived:
        raise UniverseLoadError(f"No genome surface found and no enum schema in world.yaml: {world_yaml}")
...
def _sort_key_for_objectives(metrics: dict[str, float | int], directions: dict[str, str]) -> tuple[Any, ...]:
    key_parts: list[Any] = []
    ...
    *,
    ...
    if mode == "gate_score_max":
        def sort_key(item: dict[str, Any]) -> tuple[Any, ...]:
            ...
            duration_key = float(duration_ms) if isinstance(duration_ms, (int, float)) else float("inf")
...
def _resolve_swarm(universe_cfg: UniverseConfig, swarm_id: str) -> dict[str, Any]:
    ...
    if not isinstance(swarms, dict):
        raise UniverseLoadError(f"Universe swarms.yaml missing `swarms` mapping (universe={universe_cfg.universe_id})")
    swarm = swarms.get(swarm_id)
    if not isinstance(swarm, dict):
        raise UniverseLoadError(f"Swarm not found: {swarm_id} (universe={universe_cfg.universe_id})")
    return dict(swarm)
...
def _frontier_best_parent(
    *,
    ...
    frontier_path = kernel_dir / "universes" / universe_cfg.universe_id / "frontiers" / f"{world_id}.json"
