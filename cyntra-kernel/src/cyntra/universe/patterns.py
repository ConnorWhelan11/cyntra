...
def build_patterns_store(
    *,
    ...
    for (world_id, objective_id) in sorted({(_group_key(e)) for e in evidence}, key=lambda k: (k[0] or "", k[1] or "")):
        ...
        for ngram, runs in ngram_runs.items():
            ...
            confidence = _decayed_confidence(base_confidence, last_seen_ms=last_seen_ms, reference_ms=reference_ms)
            ...
            patterns.append(
                {
                    ...
                    "outcome_distribution": {"success": successes, "failure": failures, "unknown": unknown},
