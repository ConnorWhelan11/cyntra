    ...
    if runs_dir.exists():
        for run_dir in sorted((p for p in runs_dir.iterdir() if p.is_dir()), key=lambda p: p.name):
            ...
            if evolve_world_path.exists():
                ...
                objectives = data.get("objectives") if isinstance(data.get("objectives"), dict) else None
                if isinstance(history, list):
                    for entry in history:
                        if not isinstance(entry, dict):
                            continue
                        generation = entry.get("generation") if isinstance(entry.get("generation"), int) else None
