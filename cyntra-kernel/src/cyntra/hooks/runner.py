...
class HookRunner:
    ...
    def __init__(self, config: KernelConfig) -> None:
        ...
        self._default_timeout = (
            self.hooks_config.timeout_seconds
    ...
    def _run_async_handler_sync(
        ...
        try:
            ...
            future = _executor.submit(
                lambda: asyncio.run(handler(context))
            )
            ...
            try:
                ...
                if hook.async_handler:
                    ...
                    result = await loop.run_in_executor(
                        _executor, hook.handler, context  # type: ignore
                    )
    ...
    def _should_run(self, hook: HookDefinition, context: HookContext) -> bool:
        ...
        if hook.match_tags:
            if not any(t in issue_tags for t in hook.match_tags):
                return False

        if hook.exclude_tags:
            if any(t in issue_tags for t in hook.exclude_tags):
                return False

        # Check status filter
        if hook.match_status:
            if context.proof.status not in hook.match_status:
                return False
    ...
    def get_hook_config(self, hook_name: str) -> dict[str, Any]:
        ...
        if hasattr(hook_cfg, "__dataclass_fields__"):
            return {
                k: getattr(hook_cfg, k)
                for k in hook_cfg.__dataclass_fields__
            }
