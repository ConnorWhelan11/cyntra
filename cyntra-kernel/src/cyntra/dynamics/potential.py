...
def _choose_anchor(counts: list[dict[str, Any]], state_ids: set[str]) -> str:
    totals: dict[str, int] = {state_id: 0 for state_id in state_ids}
    for row in counts:
        from_state = row.get("from_state")
...
def estimate_potential(
    ...
    *,
    ...
    potentials = {state_id: 0.0 for state_id in state_ids}
    if not edges:
        return potentials, {state_id: 0.0 for state_id in state_ids}, {
            "rmse_logratio": None,
            "edges_used": 0,
        }
    ...
    return potentials, stderr_by_state, {
        "rmse_logratio": rmse,
