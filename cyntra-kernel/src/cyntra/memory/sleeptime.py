...
from .models import AgentMemory, MemoryType, ConsolidationCluster
from .scoring import recalculate_all_scores, get_archival_candidates
...
class SleeptimeProcessor:
    ...
    def __init__(
        ...
        collective_service: Optional[CollectiveMemoryService] = None,
        consolidation_handler: Optional[ConsolidationHandler] = None,
    ):
        """
            ...
            if hub.memory_type in (MemoryType.CONTEXT, MemoryType.DYNAMIC):
                ...
                try:
                    await self.store.update(
                        memory_id=hub.id,
                        updates={"memory_type": MemoryType.PATTERN.value}
                    )
