...
def _load_scoring_formula() -> str:
    ...
    formula_path = Path(__file__).parent.parent.parent.parent / 'migrations' / '002_scoring_function.sql'
...
class MemoryStore:
    ...
    @classmethod
    async def create(cls, dsn: str) -> "MemoryStore":
        """
        Create store with new connection pool.
    ...
    async def get(self, memory_id: UUID) -> Optional[AgentMemory]:
        """
        Fetch memory by ID.
    ...
    async def increment_access(
        self,
        memory_id: UUID,
        current_runs: Optional[int] = None,
    ) -> None:
        """
    ...
    def _row_to_extraction_batch(self, row: asyncpg.Record) -> ExtractionBatch:
