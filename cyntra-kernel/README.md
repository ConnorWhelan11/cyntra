...
```bash
cyntra --help           # Show all commands
cyntra init             # Initialize kernel
cyntra run              # Run kernel loop
cyntra run --once       # Single scheduling cycle
cyntra run --dry-run    # Preview without changes
cyntra run --speculate  # Force speculate+vote mode
cyntra status           # Show status
cyntra workcells        # List workcells
cyntra history          # Show history
cyntra stats            # Show statistics
cyntra flaky-tests      # Manage flaky tests
cyntra escalate         # Manual escalation
cyntra cleanup          # Cleanup workcells
```

## Configuration

Create `.cyntra/config.yaml`:
...
Cyntra exposes an MCP server for integration with external LLM agents:

```bash
# Run MCP server
python -m cyntra.mcp.server
