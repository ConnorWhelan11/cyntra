...
- Enum: `{1, 2, 3}`
- Kernel mapping:
  - Maps to speculation parallelism in `cyntra-kernel/src/cyntra/kernel/runner.py:_dispatch_speculate_async()`
  - If `swarm_id=serial_handoff`, treat as `1` (masked/forced).
- World evolution mapping:
  - Maps to `population_size` in `cyntra-kernel/src/cyntra/universe/evolve_world.py:evolve_world()`
...
- Kernel enforcement option:
  ...
  - Teach `cyntra-kernel/src/cyntra/kernel/dispatcher.py:dispatch_async()` to prefer override over toolchain default timeout.
...
- First enforcement target (Fab iteration loops):
  - Hook into Fab iteration config where `max_iterations` is already a first-class knob (`cyntra-kernel/src/cyntra/fab/iteration.py` / `cyntra-kernel/src/cyntra/fab/config.py`).
...
Sources:
- Workcells: prefer `rollout.json` if present (`cyntra-kernel/src/cyntra/rollouts/builder.py`), else fall back to archived `manifest.json` + `proof.json`.
...
- decision boundary before speculating:
  - `cyntra-kernel/src/cyntra/kernel/runner.py:_dispatch_parallel()` (choose serial vs speculate)
- apply `max_candidates_bin`:
  - `cyntra-kernel/src/cyntra/kernel/runner.py:_dispatch_speculate_async()` (override parallelism)
- apply `max_minutes_bin` (if implemented):
  - `cyntra-kernel/src/cyntra/kernel/dispatcher.py:dispatch_async()` (override timeout)

World evolution:
- in `cyntra-kernel/src/cyntra/cli.py` evolve path (choose swarm/population budgets)
...
- Add planner inference module in `cyntra-kernel/src/cyntra/planner/`.
