...
**Schema:** `cyntra-kernel/schemas/cyntra/planner_input.schema.json` (new).
...
**Artifact:** JSON (stored for provenance).  
**Schema:** `cyntra-kernel/schemas/cyntra/planner_action.schema.json` (new).
...
- `max_candidates_bin ∈ {1,2,3,"NA"}`
  - Kernel mapping: parallelism cap in `cyntra-kernel/src/cyntra/kernel/runner.py:_dispatch_speculate_async()`
  - World mapping: `population_size` in `cyntra-kernel/src/cyntra/universe/evolve_world.py:evolve_world()`
- `max_minutes_bin ∈ {15,30,45,60,120,"NA"}`
  - Kernel mapping (v1 recommended): allow `manifest["planner"]["timeout_seconds_override"]`; use it in `cyntra-kernel/src/cyntra/kernel/dispatcher.py:dispatch_async()` to override toolchain timeout.
  - World mapping: enforce via universe/world policy envelopes or per-run override.
- `max_iterations_bin ∈ {1,2,3,5,"NA"}`
  - Fab mapping: wire into iteration config where `max_iterations` is already a first-class parameter (`cyntra-kernel/src/cyntra/fab/iteration.py` / `cyntra-kernel/src/cyntra/fab/config.py`).
  - Code jobs: `"NA"` (v1).
...
Sources:
- Workcells: prefer `rollout.json` (built by `cyntra-kernel/src/cyntra/rollouts/builder.py`), else `manifest.json` + `proof.json`.
- World runs: `.cyntra/runs/<run_id>/manifest.json` + stage metadata (as used by `cyntra-kernel/src/cyntra/universe/index.py`).

### 5.2 Similar-runs retrieval (deterministic, leakage-safe)

Module: `cyntra-kernel/src/cyntra/planner/similar_runs.py` (new).
...
Kernel issues:
- decide serial vs speculate: `cyntra-kernel/src/cyntra/kernel/runner.py:_dispatch_parallel()`
- apply parallelism cap: `cyntra-kernel/src/cyntra/kernel/runner.py:_dispatch_speculate_async()`
- apply timeout override: `cyntra-kernel/src/cyntra/kernel/dispatcher.py:dispatch_async()`

World evolution:
- evolve CLI path (choose swarm/population/budgets): `cyntra-kernel/src/cyntra/cli.py` evolve command.
