...
**Date:** 2025-12-25
**Scope:** `apps/glia-fab-desktop` viewport window + render loop
...
- IPC stays **in-process** and should be treated as a *bounded* producer/consumer system (explicit coalescing before `emit`), not “Tauri events are backpressured”.
...
- The repository is currently Tauri v1; this ADR assumes we first migrate `apps/glia-fab-desktop` to Tauri v2 (tracked separately).
