...
This guide helps you migrate from the legacy `fab/assets/` pipeline to the new Fab World system at `fab/worlds/outora_library/`.
...
1. **Archive old blend files**:
   ...
   mv fab/assets/blender/*.blend ~/Archives/
   ```

2. **Deprecate old scripts**:
   - Add deprecation warnings to `fab/assets/blender/*.py`
...
| ----------------------------- | ------------------------------------------- |
| `fab/assets/blender/*.blend1` | Gitignored                                  |
| `fab/assets/exports/*.glb`    | `.cyntra/runs/<run_id>/world/` (gitignored) |
| Manual `/tmp/` usage          | `FAB_RUN_DIR`, `FAB_STAGE_DIR` env vars     |
| `fab/assets/viewer/assets/`   | Published via `fab-world publish --viewer`  |
...
2. **Preserve old artifacts**:
   ...
   tar -czf outora-library-backup-$(date +%Y%m%d).tar.gz fab/assets/
   ```
