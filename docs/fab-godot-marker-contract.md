...
1. **Blender `navmesh.py` Stage:** Generates NAV_ meshes from floor geometry
2. **Blender `export.py` Stage:** Adds NPC_SPAWN_, WAYPOINT_ markers
...
- At least 1 COLLIDER_ marker exists (if `require_colliders: true`)
- NAV_ markers exist (if `require_navmesh: true`)
- NPC_SPAWN_ markers exist (if `require_npc_spawns: true`)
...
- NPC_SPAWN_ markers spawn CogitoNPC instances
- ITEM_SPAWN_ markers spawn Cogito interactables
- Patrol paths are assigned to NPCs with `set_patrol_path()`
- Audio zones integrate with Cogito's audio system

## Version History

- **v1.1** (2024-12): Added NAV_, NPC_SPAWN_, ITEM_SPAWN_, AUDIO_ZONE_, WAYPOINT_ markers
- **v1.0** (2024-11): Initial contract with SPAWN_PLAYER, COLLIDER_, TRIGGER_, INTERACT_
