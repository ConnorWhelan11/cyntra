...
- kernel/ — Python “Cyntra Kernel” orchestrator (this is the real product code).
...
- Core orchestration loop (the “kernel”)
  - kernel/src/cyntra/kernel/runner.py — main loop: schedule → dispatch → verify
    → update Beads → cleanup.
  - kernel/src/cyntra/kernel/scheduler.py — ready set + critical path + lane
    packing; uses Issue.dk_attempts/dk_max_attempts.
  - kernel/src/cyntra/kernel/dispatcher.py — creates workcells + writes
    manifest.json + invokes adapters + merges to main.
  - kernel/src/cyntra/kernel/verifier.py — runs “quality gates” and decides pass/
    fail; currently hardwired to test/typecheck/lint.
- Workcell isolation + lifecycle
  - kernel/src/cyntra/workcell/manager.py — git worktrees + archives logs (note:
...
- Quality gate runner (reusable for “Realism Gate” if treated as a gate command)
  - kernel/src/cyntra/gates/runner.py — runs shell commands + logs; suitable
    wrapper for invoking Blender headless + critics script.
- Observability
  - kernel/src/cyntra/observability/events.py — event schema + reader; kernel
    currently logs events via StateManager.add_event() (kernel/src/cyntra/state/
    manager.py).
- Toolchain adapters (agent execution)
  - kernel/src/cyntra/adapters/\*.py — Codex/Claude/Crush adapters; compute diffs
    - forbidden path violations deterministically.
- Schemas (existing)
  - kernel/schemas/manifest.schema.json — workcell input manifest schema (draft-07).
  - kernel/schemas/proof.schema.json — Patch+Proof output schema (draft-07).

Config state (important mismatch)

- Code parses a flat config model: kernel/src/cyntra/kernel/config.py.
- Docs/examples describe a richer nested config (quality_gates:, routing:, etc) but code
  does not implement that:
  - kernel/docs/guides/configuration.md
  - kernel/examples/config.yaml, kernel/examples/config-python.yaml, kernel/
...
- Canonical headless render harness (Blender)
  - ❌ Missing (no Blender invocation code anywhere under kernel/src/cyntra/).
...
- Determinism + audit artifacts + structured JSON reports
  - 🟨 Partially exists:
    - Deterministic orchestration + logging exists (kernel/src/cyntra/kernel/
      \*.py, kernel/src/cyntra/state/manager.py).
...
- Gate decision logic (kernel-owned pass/fail)
  - 🟨 Partially exists:
    - Verifier exists (kernel/src/cyntra/kernel/verifier.py) but only runs code
      gates (test/typecheck/lint) and doesn’t know about asset gates.
- Iteration loop (generate → render → score → repair)
  - 🟨 Partially exists:
    - Attempts counters exist (Issue.dk_attempts/dk_max_attempts in kernel/src/
      cyntra/state/models.py, and scheduler checks them in kernel/src/
      cyntra/kernel/scheduler.py).
    - But kernel failure path sets status to "blocked" (kernel/src/cyntra/
...
- Beads integration and work graph logging
  - ✅ Exists (kernel/src/cyntra/state/manager.py, kernel/src/cyntra/
    state/models.py).
- Workcell contracts / schemas
  - ✅ Exists for code tasks (kernel/schemas/manifest.schema.json, kernel/
...
|
v
KernelRunner (kernel/src/cyntra/kernel/runner.py)
|
...
|
+--> Verifier (verifier.py) [needs extension] - runs GateRunner commands - NEW: run `fab-realism` gate for asset-tagged issues
...
- Code: kernel/src/cyntra/fab/ (render harness + critic(s) + gate runner)
- Schemas: kernel/schemas/fab/
...
- Owner: Kernel
- Targets (new):
  - kernel/src/cyntra/fab/gate.py (entry point: run render harness + critic(s) +
    emit report JSON + exit 0/1)
  - kernel/src/cyntra/fab/config.py (loads gate config + lookdev paths)
- Acceptance:
...
- Owner: Kernel
- Targets (edit):
  - kernel/src/cyntra/kernel/dispatcher.py — include issue tags in manifest.json
    (currently tags are not included), and add/override quality_gates when tags include
    asset:\* / gate:realism.
  - kernel/src/cyntra/kernel/verifier.py — stop hardwiring gates to test/
...
- Owner: Kernel
- Targets (edit):
  - kernel/src/cyntra/workcell/manager.py — archive logs/ recursively (today it
...
- Owner: Kernel
- Targets (edit):
  - kernel/src/cyntra/kernel/verifier.py and/or kernel/src/cyntra/kernel/
...
- Targets (new):
  - kernel/src/cyntra/fab/critics/ (category/alignment/realism/geometry)
...
- Owner: Kernel
- Targets (edit):
  - kernel/src/cyntra/kernel/runner.py failure path: avoid permanent "blocked"
    for retryable gate failures; either re-open the issue or create a child “repair”
    issue with dependency edges.
  - kernel/src/cyntra/state/manager.py updates: add fields like
...
- Keep consistency with existing draft-07 schemas in kernel/schemas/:
  - kernel/schemas/fab/asset-proof.schema.json
  - kernel/schemas/fab/critic-report.schema.json
  - kernel/schemas/fab/gate-verdict.schema.json

Versioning

- Follow existing schema_version: "1.0.0" pattern (see kernel/schemas/
  proof.schema.json).
...
- Kernel validates reports using existing dependency jsonschema (kernel/pyproject.toml)
...
- Files:
  ...
  - kernel/schemas/fab/gate-verdict.schema.json
- Validates:
  - python -c "import json; json.load(open('kernel/schemas/fab/gate-
    verdict.schema.json'))"
...
- Files:
  - kernel/src/cyntra/fab/gate.py
  - kernel/src/cyntra/fab/config.py
- Validates:
...
- Files:
  - kernel/src/cyntra/kernel/dispatcher.py (include issue.tags; set quality_gates
    based on tags)
  - kernel/src/cyntra/kernel/verifier.py (run gates from
    manifest.json["quality_gates"])
- Validates:
  - pytest -q under kernel/ (existing integration/unit tests)

5. Commit: “Archive render trees + persist verified proof.json”

- Files:
  - kernel/src/cyntra/workcell/manager.py (recursive logs archiving)
  - kernel/src/cyntra/kernel/verifier.py (write updated proof back to disk)
- Validates:
