...
```bash
fab-world publish --run .cyntra/runs/<selected_run_id> --viewer fab/outora-library/viewer
```

Then serve the viewer over HTTP:

```bash
(cd fab/outora-library/viewer && python3 -m http.server 8000)
```
...
```bash
fab-world publish --run .cyntra/runs/<selected_run_id> --export fab/outora-library/export/releases/
```
