...
- `docs/plans/outora-library-plan1.md`: Architecture notes and historical plan references.
- `kernel/src/cyntra/fab/outora/`: Python helper package for the asset pipeline.
- `fab/assets/blender/`: Automation (e.g., `integrate_assets.py`, `apply_style.py`) and the working `.blend`; run these via Blender, not straight with CPython.
- `fab/assets/models/`, `fab/assets/textures/`, `fab/assets/hdris/`, `fab/assets/room1107/`: Source assets; mirror any new subfolders in `fab/assets/licenses/` for attribution.
- `fab/assets/exports/`: Generated GLB/FBX or baked outputs ready for downstream use.

## Environment Setup, Build, and Test

- Python 3.11+. Use the kernel environment (`cd kernel && pip install -e ".[dev,fab]"`).
- Lint/format: `cd kernel && ruff check src/cyntra/fab/outora` (format via `ruff format` if needed).
- Tests: `cd kernel && pytest tests/fab/`; prefer mocking `bpy` when testing Blender-facing logic.
- Blender automation (headless example): `blender fab/assets/blender/outora_library_v0.4.0.blend --background --python fab/assets/blender/integrate_assets.py`; copy the blend before experiments.
