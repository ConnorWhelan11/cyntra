...
try:
    ...
    desk_count = sum(
        1 for obj in bpy.context.scene.objects if "desk" in obj.name.lower()
    )
    chair_count = sum(
        1 for obj in bpy.context.scene.objects if "chair" in obj.name.lower()
    )
