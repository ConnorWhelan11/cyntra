...
def create_study_pod(config):
    ...
    if desk_src:
        desk = instance_object(
            desk_src, f"{pod_col_name}_desk", pos, rot_z, (1.35, 1.35, 1.35)
        )
    else:
        ...
        for i in range(num_stacks):
            ...
            if book_src:
                ...
                stack = create_book_stack(
                    f"{pod_col_name}_books_{i}", num_books, seed + i
                )
    ...
    if lamp_src:
        lamp = instance_object(
            lamp_src, f"{pod_col_name}_lamp", lamp_pos, rot_z, (1.0, 1.0, 1.0)
        )
        if lamp:
