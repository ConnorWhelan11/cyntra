...
def create_desk(name: str, location: tuple, rotation_z: float = 0) -> bpy.types.Object:
    ...
    for i, (lx, ly) in enumerate(leg_positions):
        bpy.ops.mesh.primitive_cube_add(
            size=1, location=(lx, ly, height / 2 - thickness)
        )
...
def create_chair(name: str, location: tuple, rotation_z: float = 0) -> bpy.types.Object:
    ...
    # Back
    bpy.ops.mesh.primitive_cube_add(
        size=1, location=(0, seat_size / 2 - 0.02, seat_height + back_height / 2)
    )
    ...
    for i, (lx, ly) in enumerate(leg_positions):
        bpy.ops.mesh.primitive_cylinder_add(
            radius=0.02,
            depth=seat_height - 0.03,
            location=(lx, ly, seat_height / 2 - 0.015),
        )
        leg = bpy.context.active_object
...
def create_bookshelf(
    name: str, location: tuple, rotation_z: float = 0
) -> bpy.types.Object:
    """Create a bookshelf."""
    width = 1.0
    depth = 0.35
    ...
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cube_add(
            size=1, location=(side * (width / 2 - thickness / 2), 0, height / 2)
        )
