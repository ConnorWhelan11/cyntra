...
class GothicLayoutGenerator:
    ...
    def __init__(
        ...
        self.wing_extent = (
            self.half_crossing + wing_length_bays * bay
        )  # 36m from center
    ...
    def mat(
        self, x: float, y: float, z: float = 0.0, rot_z: float = 0.0, scale: float = 1.0
    ) -> mathutils.Matrix:
    ...
    def is_in_plan(self, bx: int, by: int) -> bool:
        ...
        if (
            abs(bx) <= wing_half_width
            and abs(by) <= self.crossing_bays // 2 + self.wing_length_bays
        ):
            return True

        # East/West wings (along X axis)
        if (
            abs(by) <= wing_half_width
            and abs(bx) <= self.crossing_bays // 2 + self.wing_length_bays
        ):
            return True
    ...
    def get_zone(self, bx: int, by: int) -> str:
        ...
        if abs(bx) <= half_cross and abs(by) <= half_cross:
            if (
                abs(bx) <= self.nave_width_bays // 2
                and abs(by) <= self.nave_width_bays // 2
            ):
    ...
    def is_on_nave_axis(self, bx: int, by: int) -> bool:
        ...
        return (
            abs(bx) <= self.nave_width_bays // 2 or abs(by) <= self.nave_width_bays // 2
        )
    ...
    def generate(self):
        ...
        for bx in range(-max_extent, max_extent + 1):
            for by in range(-max_extent, max_extent + 1):
                ...
                elif (is_main_pier_line_x or is_main_pier_line_y) and self.is_in_plan(
                    bx, by
                ):
                    if zone not in ["crossing_nave", "nave_x", "nave_y"]:
                        self.arcade_columns.append(self.mat(x, y, 0))
                        self.arcade_columns.append(
                            self.mat(x, y, TRIFORIUM_LEVEL, scale=0.8)
                        )
        ...
        for bx in range(-max_extent, max_extent + 1):
            for by in range(-max_extent, max_extent + 1):
                ...
                if zone in ["aisle_x", "aisle_y", "crossing_aisle"]:
                    ...
                    self.mezzanine_beams.append(
                        self.mat(x, y, TRIFORIUM_LEVEL - BEAM_DEPTH / 2)
                    )
        ...
        for bx in range(-max_extent, max_extent + 1):
            for by in range(-max_extent, max_extent + 1):
                ...
                if is_edge:
                    ...
                    # Clerestory lancets
                    self.lancet_windows_clerestory.append(
                        self.mat(x, y, CLERESTORY_BASE + 1.0, rot)
                    )
        ...
        for sign_x in [-1, 1]:
            for sign_y in [-1, 1]:
                ...
                self.spiral_stairs.append(
                    self.mat(
                        sign_x * wing_half,
                        sign_y
                        * (self.half_crossing + self.wing_length_bays * self.bay * 0.8),
                        0,
                    )
                )
...
def create_layout_directly():
    ...
    def create_empties(
        name: str, matrices: List, parent_col, display_type="PLAIN_AXES", size=0.5
    ):
    ...
    create_empties(
        "CrossingArches", outputs["crossing_arches"], root_col, "SPHERE", 1.5
    )
    create_empties("NaveArches", outputs["nave_arches"], root_col, "SPHERE", 1.0)
    create_empties(
        "ExteriorWalls", outputs["exterior_walls"], root_col, "PLAIN_AXES", 3.0
    )
    create_empties("GroundFloor", outputs["ground_floor"], root_col, "PLAIN_AXES", 0.3)
    create_empties(
        "MezzanineFloor", outputs["mezzanine_floor"], root_col, "PLAIN_AXES", 0.3
    )
    ...
    create_empties(
        "Chandeliers", outputs["chandelier_positions"], root_col, "SPHERE", 0.8
    )
...
def print_layout_summary():
    ...
    print(
        f"   Crossing: {CROSSING_BAYS}x{CROSSING_BAYS} bays ({CROSSING_BAYS * BAY}m x {CROSSING_BAYS * BAY}m)"
    )
    ...
    print(
        f"   Wing length: {WING_LENGTH_BAYS} bays ({WING_LENGTH_BAYS * BAY}m beyond crossing)"
    )
