...
def get_or_create_material(name: str, config: dict):
    ...
    if bsdf:
        bsdf.inputs["Base Color"].default_value = config.get(
            "color", (0.5, 0.5, 0.5, 1)
        )
...
def create_clustered_pier(
    ...
    for i in range(num_shafts):
        ...
        # Move colonette
        bmesh.ops.translate(
            col_bm, verts=col_bm.verts, vec=(x, y, base_z + shaft_height / 2)
        )
    ...
    if with_base:
        ...
        bmesh.ops.translate(
            base_bm, verts=base_bm.verts, vec=(0, 0, PIER_BASE_HEIGHT * 0.2)
        )
        ...
        bmesh.ops.translate(
            base_bm, verts=[v for v in base_bm.verts if v.co.z < 0.1], vec=(0, 0, 0.05)
        )
    ...
    if with_capital:
        ...
        bmesh.ops.translate(
            cap_bm,
            verts=[
                v for v in cap_bm.verts if v.co.z < cap_z + PIER_CAPITAL_HEIGHT * 0.5
            ],
...
def create_rose_window(
    ...
    for i in range(num_petals):
        ...
        spoke_verts = [
            bm.verts.new(
                (
                    x_dir * hub_radius - z_dir * spoke_width,
                    0,
            ...
            bm.verts.new(
                (
                    x_dir * inner_radius + z_dir * spoke_width,
                    0,
...
def create_balustrade_section(
    ...
    for i in range(num_balusters):
        ...
        # Move to position
        bmesh.ops.translate(
            col_bm, verts=col_bm.verts, vec=(x, 0, rail_height + baluster_height / 2)
        )
...
def generate_all_pieces(collection_name: str = "OL_Gothic_Kit"):
    ...
    pieces["GK_VaultRib"] = create_vault_rib("GK_VaultRib", span=6.0, rise=2.0)
    print("   ✅ Vault Rib")
