    ...
    for col_name in kit_collections:
        ...
        if col:
            ...
            for child in col.children:
                if select_collection_objects(child.name):
                    ...
                    if selected > 0:
                        ...
                        export_glb(filepath)
                        print(
                            f"    {child.name}: {selected} objects -> {filepath.name}"
                        )
                        exported += 1
            ...
            if select_collection_objects(col_name):
                selected = len([o for o in bpy.context.selected_objects])
                if selected > 0:
                    filepath = OUTPUT_DIR / "kit" / "full_kit.glb"
                    export_glb(filepath)
...
def export_wings():
    ...
    for wing_name, bounds in wings.items():
        ...
        for obj in bpy.data.objects:
            ...
            loc = obj.location
            if (
                min_b[0] <= loc.x <= max_b[0]
                and min_b[1] <= loc.y <= max_b[1]
                and min_b[2] <= loc.z <= max_b[2]
            ):
                obj.select_set(True)
