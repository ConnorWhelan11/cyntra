...
def create_gothic_megalibrary():
    ...
    arch_offset = 9.0
    place_copy(
        "GIK_Arch1", "ol_arch_n", (0.0, arch_offset, 0.0), (0.0, 0.0, 0.0), arch_scale
    )
    place_copy(
        "GIK_Arch1",
        "ol_arch_s",
        (0.0, -arch_offset, 0.0),
        (0.0, 0.0, math.radians(180)),
        arch_scale,
    )
    place_copy(
        "GIK_Arch1",
    ...
    for x in edge_positions:
        place_copy(
            wall_src, f"ol_wall_n_{idx}", (x, 18.0, 0.0), (0.0, 0.0, 0.0), wall_scale
        )
        idx += 1
    idx = 0
    for x in edge_positions:
...
def create_blockout():
    ...
    bmesh.ops.create_circle(
        ...
        cap_ends=True,
        radius=1.0,
        segments=SEGMENTS * 2,  # Smoother floor
    )
    # Scale to oval
