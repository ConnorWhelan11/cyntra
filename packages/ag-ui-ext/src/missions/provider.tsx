...
export function MissionRuntimeProvider({
  ...
  const [definition, setDefinition] = useState<MissionDefinition | null>(initialDefinition ?? null);
  ...
  const eventLog = useMemo(() => state?.eventLog ?? [], [state?.eventLog]);
  ...
  return (
    <MissionRuntimeContext.Provider value={contextValue}>{children}</MissionRuntimeContext.Provider>
  );
}
...
export function useMissionToolContext(_toolId: MissionToolId): MissionToolContext | null {
