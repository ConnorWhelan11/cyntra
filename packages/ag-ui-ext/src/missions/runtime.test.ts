...
describe("missionReducer - lifecycle events", () => {
  ...
  it("mission/pause transitions from active to paused", () => {
    // Start first
    let nextState = missionReducer(
      state,
      { type: "mission/start", runId: state.runId, at: Date.now() },
      testDefinition
    );

    // Then pause
    nextState = missionReducer(
      nextState,
      { type: "mission/pause", runId: state.runId, at: Date.now() },
      testDefinition
    );
  ...
  it("mission/resume transitions from paused to active", () => {
    ...
    nextState = missionReducer(
      nextState,
      { type: "mission/resume", runId: state.runId, at: Date.now() + 5000 },
      testDefinition
    );
  ...
  it("mission/abort transitions to aborted", () => {
    ...
    nextState = missionReducer(
      nextState,
      { type: "mission/abort", runId: state.runId, at: Date.now(), reason: "User cancelled" },
      testDefinition
    );
  ...
  it("mission/complete transitions to completed", () => {
    ...
    nextState = missionReducer(
      nextState,
      { type: "mission/complete", runId: state.runId, at: Date.now() },
      testDefinition
    );
...
describe("missionReducer - step events", () => {
  ...
  it("step/activate sets active step", () => {
    ...
    nextState = missionReducer(
      nextState,
      { type: "step/activate", runId: state.runId, at: Date.now(), stepId: "step-2" },
      testDefinition
    );
  ...
  it("completing last step transitions to debrief phase", () => {
    ...
    nextState = missionReducer(
      nextState,
      { type: "step/complete", runId: state.runId, at: Date.now(), stepId: "step-2" },
      testDefinition
    );
    nextState = missionReducer(
      nextState,
      { type: "step/activate", runId: state.runId, at: Date.now(), stepId: "step-3" },
      testDefinition
    );
    nextState = missionReducer(
      nextState,
      { type: "step/complete", runId: state.runId, at: Date.now(), stepId: "step-3" },
      testDefinition
    );
...
describe("missionReducer - checkpoint events", () => {
  ...
  it("checkpoint/ack clears checkpoint and returns to running", () => {
    ...
    nextState = missionReducer(
      nextState,
      { type: "checkpoint/ack", runId: state.runId, at: Date.now(), checkpointId: "cp-1" },
      testDefinition
    );
...
describe("isStepCompletionSatisfied", () => {
  ...
  it("evaluates anyOf completion", () => {
    const completion = {
      kind: "anyOf" as const,
      of: [{ kind: "time" as const, seconds: 60 }, { kind: "manual" as const }],
    };
...
describe("canTransitionTo", () => {
  ...
  it("allows active -> paused", () => {
    expect(canTransitionTo({ status: "active", phase: "running" }, { status: "paused" })).toBe(
      true
    );
  });

  it("allows paused -> active", () => {
    expect(canTransitionTo({ status: "paused", phase: "running" }, { status: "active" })).toBe(
      true
    );
  });

  it("disallows completed -> active", () => {
    expect(canTransitionTo({ status: "completed", phase: "debrief" }, { status: "active" })).toBe(
      false
    );
  });
});
