...
export function createInitialState(
  ...
  const openToolIds = definition.tools.filter((t) => t.placement?.defaultOpen).map((t) => t.toolId);
...
export function isStepCompletionSatisfied(
  ...
  switch (completion.kind) {
    ...
    case "anyOf":
      return completion.of.some((c) => isStepCompletionSatisfied(c, stepState, elapsedStepSeconds));
...
export function getNextStepId(state: MissionState): MissionStepId | null {
  const activeIndex = state.activeStepId ? state.stepOrder.indexOf(state.activeStepId) : -1;
