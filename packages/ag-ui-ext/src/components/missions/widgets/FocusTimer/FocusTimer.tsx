...
export function FocusTimer({ showCountdown = true, size = "md", className }: FocusTimerProps) {
  const { definition, state, elapsedSeconds, isPaused, isActive, pauseMission, resumeMission } =
    useMissionRuntime();
  ...
  return (
    <div className={cn("focus-timer", className)}>
      ...
      {hasStepTimer && currentStep?.completion.kind === "time" && (
        <div className="mt-3 pt-3 border-t border-border/40">
          <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
            <span>Step time</span>
            <span>
              {formatTime(stepState?.elapsedSeconds ?? 0)} /{" "}
              {formatTime(currentStep.completion.seconds)}
            </span>
          </div>
      ...
      {/* Paused indicator */}
      {isPaused && <div className="mt-2 text-xs text-amber-400">⏸ Paused</div>}
    </div>
  );
}
