...
import { ChevronRight, Clock, Pause, Play, SkipForward, Square, Users } from "lucide-react";
...
function PhaseIndicator({ phase, status }: PhaseIndicatorProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className={cn("h-2 w-2 rounded-full", statusColors[status] ?? "bg-slate-500")} />
      <span
        className={cn(
    ...
    <div className="flex flex-col gap-1">
      {definition.tools.map((toolRef) => {
        ...
        return (
          ...
          >
            {tool.icon ?? (
              <span className="text-xs font-medium">{tool.title.slice(0, 2).toUpperCase()}</span>
            )}
          </button>
        );
      })}
      ...
      return (
        <tool.Panel
          ...
          context={{
            ...
            state,
            dispatch,
            stepState: state.activeStepId ? state.steps[state.activeStepId] : undefined,
          }}
        />
      );
    },
    ...
    <div className={cn("mission-shell flex h-full flex-col", className)}>
      ...
      >
        ...
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-lg font-semibold text-foreground">{definition.title}</h1>
            <div className="flex items-center gap-2 mt-0.5">
              <span
                className={cn(
                  "flex items-center gap-1 text-[10px] uppercase tracking-wider",
                  definition.mode === "pod" ? "text-magenta-neon" : "text-cyan-neon"
                )}
              >
                {definition.mode === "pod" ? <Users className="h-3 w-3" /> : null}
                {definition.mode}
              </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {isActive && !isPaused && (
            <>
              <GlowButton variant="outline" glow="none" size="sm" onClick={pauseMission}>
                <Pause className="mr-1.5 h-3.5 w-3.5" />
                Pause
              </GlowButton>

              {state.activeStepId && (
                <GlowButton variant="ghost" glow="none" size="sm" onClick={completeCurrentStep}>
      ...
      {/* Dev Panel */}
      {showDevPanel && <DevPanel eventLog={eventLog} state={state} />}
    </div>
  );
}
...
function CheckpointModal({ checkpoint, definition, shouldAnimate }: CheckpointModalProps) {
  const { ackCheckpoint } = useMissionRuntime();
  const checkpointDef = definition.checkpoints?.find((c) => c.id === checkpoint.id);
    ...
    >
      ...
      >
        ...
        <div className="flex justify-end gap-2">
          <GlowButton glow="low" onClick={() => ackCheckpoint(checkpoint.id)}>
            Continue
