...
export function PracticeQuestionPanel({
  ...
  return (
    <div className={cn("rounded-2xl border border-border/40 bg-card/40 p-5", className)}>
      ...
      <p className="mt-2 text-sm font-medium text-foreground">{panel.title ?? "Practice"}</p>

      <p className="mt-4 whitespace-pre-wrap text-sm text-foreground">{panel.props.prompt}</p>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs text-muted-foreground">reveal: {revealMode}</div>
        <div className="flex items-center gap-2">
          ...
          <Button disabled={!selected} onClick={() => setSubmitted(true)} className="gap-2">
            Submit
          </Button>
        </div>
      </div>
