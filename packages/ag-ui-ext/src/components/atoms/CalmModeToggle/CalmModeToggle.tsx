...
const calmModeToggleVariants = cva(
  "inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default: "border border-border bg-card hover:bg-accent hover:text-accent-foreground",
...
export function CalmModeToggle({
  ...
  const handleToggle = () => {
    const newIsCalm = controlledIsCalm !== undefined ? !controlledIsCalm : !internalIsCalm;
  ...
  const isCalm = controlledIsCalm !== undefined ? controlledIsCalm : internalIsCalm;
  ...
  const currentIcon = isCalm ? calmIcon || defaultCalmIcon : normalIcon || defaultNormalIcon;
  ...
  return (
    <GlowButton
      ...
      size={size}
      glow="none"
      className={cn(calmModeToggleVariants({ variant, size }), "group relative", className)}
    ...
    >
      ...
      <motion.div
        className={cn(
          ...
          isCalm ? "border-emerald-500/50 bg-emerald-500/10" : "border-cyan-neon/30 bg-cyan-neon/5"
        )}
