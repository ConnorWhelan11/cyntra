...
const TimelineBlock: React.FC<TimelineBlockProps> = ({ block, isActive, onClick }) => {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className="font-medium text-white/90 truncate">{block.label}</span>
            <span className="text-xs text-white/50 whitespace-nowrap">{block.duration}m</span>
          </div>
          {block.description && (
            <p className="text-xs text-white/50 mt-1 truncate">{block.description}</p>
          )}
          <div className="text-[10px] text-white/40 mt-1">{formatTime(block.scheduledStart)}</div>
        </div>
        {block.status === "done" && <span className="text-green-400">✓</span>}
        {block.status === "active" && <span className="text-cyan-400 animate-pulse">◉</span>}
      </div>
    </button>
  );
};
...
const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  const sortedBlocks = useMemo(
    () => [...blocks].sort((a, b) => a.scheduledStart.getTime() - b.scheduledStart.getTime()),
    [blocks]
  );

  // Find where to insert the Now marker
  const nowMs = currentTime.getTime();
  const nowIndex = sortedBlocks.findIndex((b) => b.scheduledStart.getTime() > nowMs);
        ...
        <div className="space-y-2">
          {sortedBlocks.map((block, index) => (
            <React.Fragment key={block.id}>
              ...
              {nowIndex === index && <NowMarker currentTime={currentTime} blocks={blocks} />}
              <TimelineBlock
                block={block}
...
const RingView: React.FC<RingViewProps> = ({ blocks, activeBlockId, onBlockTap }) => {
...
export const TodayLensScene: React.FC<TodayLensSceneProps> = ({
  ...
  const glyphPosition: [number, number, number] = viewMode === "ring" ? [0, 2, 0] : [3, 0, 2];
  ...
  return (
    <>
      ...
      {viewMode === "timeline" ? (
        ...
        <RingView blocks={blocks} activeBlockId={effectiveActiveId} onBlockTap={onBlockTap} />
      )}
      ...
      {/* Post Processing */}
      <EffectComposer>
        <Bloom mipmapBlur intensity={0.2} luminanceThreshold={0.85} luminanceSmoothing={0.7} />
      </EffectComposer>
    </>
  );
};
