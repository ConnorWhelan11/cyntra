...
> **Reuse the existing calendar module at `packages/ag-ui-ext/src/components/calendar`.**
...
```
packages/ag-ui-ext/src/components/calendar/
...
```css
/* packages/ag-ui-ext/src/components/calendar/week-lens-overrides.css */
...
const blockToCalendarEvent = (block: ScheduledBlock, weekStart: Date): IEvent => {
...
const handleCalendarDrop = (eventId: number, newDate: Date, newHour: number) => {
...
- **Calendar reuse:** Adapt `CalendarWeekView` from `packages/ag-ui-ext/src/components/calendar/views/week-and-day-view/`.
