  ...
  return (
    <div className="absolute inset-0 opacity-15 pointer-events-none">
      ...
      >
        ...
        />
        <EffectComposer>
          <Bloom mipmapBlur intensity={0.3} luminanceThreshold={0.6} luminanceSmoothing={0.8} />
        </EffectComposer>
      </Canvas>
    </div>
  );
...
const BlockCard: React.FC<BlockCardProps> = ({ block, onHover, onRemove, onDoubleClick }) => {
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className={cn("text-xs font-medium truncate", colors.text)}>{block.label}</div>
...
const DayColumn: React.FC<DayColumnProps> = ({
  ...
  const dayBlocks = blocks.filter((b) => b.dayIndex === dayIndex).sort((a, b) => a.order - b.order);

  return (
    <div className={cn("flex flex-col min-w-0", isToday && "ring-1 ring-cyan-400/30 rounded-lg")}>
      {/* Header */}
      <button
        onClick={onDayClick}
        className={cn(
      ...
      >
        <div className={cn("text-xs font-medium", isToday ? "text-cyan-300" : "text-white/60")}>
          {DAY_NAMES[dayIndex]}
        </div>
        <div className={cn("text-[10px]", isToday ? "text-cyan-400" : "text-white/40")}>
          {date.getDate()}
        </div>
      </button>
...
export const WeekLens: React.FC<WeekLensProps> = ({
  ...
  // Get Glyph context
  const glyphContext = useMemo(() => getWeekGlyphContext(schedule, goalBias), [schedule, goalBias]);
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-white/5 bg-black/30 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          <span className="text-sm font-medium text-white/90">← {formatWeekRange(weekStart)}</span>
        </div>
      ...
      <div className="relative z-10 px-6 py-3 border-t border-white/5 bg-black/30 backdrop-blur-sm">
        <div className="flex items-center gap-2 overflow-x-auto">
          <span className="text-[10px] uppercase tracking-wider text-white/40 shrink-0">Add:</span>
          {suggestions.map((s) => (
            <SuggestionChip key={s.id} suggestion={s} onClick={() => handleSuggestionClick(s)} />
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 px-6 py-4 border-t border-white/5 bg-black/30 backdrop-blur-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-white/30">
            {schedule.length} blocks · {dayHours.reduce((a, b) => a + b, 0).toFixed(1)}h total
          </div>
        </div>
      </div>
    </div>
