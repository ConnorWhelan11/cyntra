...
const routeToExam = [NOW_NODE_ID, missionRingNodes[0].id, missionRingNodes[1].id, EXAM_NODE_ID];
...
const InteractiveGraphLens: React.FC<Partial<GraphLensProps>> = (props) => {
  ...
  const [focusNodeId, setFocusNodeId] = useState<string | undefined>(props.focusNodeId);
  ...
  const handlePlanRequest = useCallback((context: { focusNodeId: string; goalNodeId?: string }) => {
    console.log("Plan requested:", context);
    alert(`Planning from ${context.focusNodeId} to ${context.goalNodeId || "goal"}`);
  }, []);
...
export const Overview: Story = {
  render: () => <InteractiveGraphLens mode="overview" focusNodeId={NOW_NODE_ID} />,
  parameters: {
    docs: {
      description: {
        story:
...
export const AttentionLeaks: Story = {
  render: () => (
    <GraphLens
      ...
      mode="attentionLeaks"
      focusNodeId={NOW_NODE_ID}
      distractionNodeIds={[DISTRACTION_YOUTUBE, DISTRACTION_TWITTER, DISTRACTION_REDDIT]}
    />
  ),
  parameters: {
    docs: {
...
export const MissionRing: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Mission nodes in a ring layout representing a focus session timeline.",
      },
    },
  },
};
...
export const GoalsFocus: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Focused on the Med School goal with other goals and related tasks highlighted.",
      },
    },
  },
};
