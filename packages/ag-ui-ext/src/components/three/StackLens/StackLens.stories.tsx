...
import type { EnforcementState, StackLensProps, StackTask, TaskStatus } from "./types";
...
const sampleTasks: StackTask[] = [
  createTask("task-1", "Orgo Review", "Chapter 12, Practice Problems", 45, "pending"),
  createTask("task-2", "Practice Problems Set 3", "Finish remaining 10 problems", 30, "pending"),
  createTask("task-3", "Review Chapter 11", "Quick review if time permits", 20, "pending"),
];
...
const InteractiveStackLens: React.FC<InteractiveStackLensProps> = ({
  ...
  const handleTaskStart = useCallback((taskId: string) => {
    setTasks((prev) =>
      prev.map((t) =>
        t.id === taskId ? { ...t, status: "active" as TaskStatus, startedAt: new Date() } : t
      )
    );
  ...
  const handleTaskComplete = useCallback((taskId: string) => {
    setTasks((prev) =>
      prev.map((t) =>
        t.id === taskId ? { ...t, status: "done" as TaskStatus, completedAt: new Date() } : t
      )
    );
  ...
  const handleTaskSkip = useCallback((taskId: string) => {
    setTasks((prev) =>
      prev.map((t) => (t.id === taskId ? { ...t, status: "skipped" as TaskStatus } : t))
    );
...
export const Default: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Default stack view with 3 tasks ready to start. Click 'Start Timer' to begin.",
      },
    },
  },
};
...
export const WithEnforcement: Story = {
  render: () => <InteractiveStackLens enforcementState={mockEnforcement} />,
  parameters: {
    docs: {
      description: {
        story: "Stack with enforcement badge showing blocked sites and remaining time.",
      },
    },
  },
};
...
export const AllComplete: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Stack cleared! Shows celebration state and prompts for Debrief.",
      },
    },
  },
};
...
export const Interactive: Story = {
  render: () => (
    <InteractiveStackLens initialTasks={sampleTasks} enforcementState={mockEnforcement} />
  ),
  parameters: {
    docs: {
      description: {
        story: "Full interactive demo. Start timer, complete/skip tasks, see all states.",
      },
    },
  },
};
