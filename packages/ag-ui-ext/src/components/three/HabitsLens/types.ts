 ...
 * Instance status
 */
export type HabitInstanceStatus = "scheduled" | "in_progress" | "completed" | "partial" | "missed";
...
export function getHabitsGlyphContext(
  ...
  if (streakData) {
    ...
    const strongStreaks = Array.from(streakData.values()).filter((s) => s.currentStreak >= 7);
