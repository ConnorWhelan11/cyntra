...
const StepCircle: React.FC<StepCircleProps> = ({ step, status = "pending" }) => {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-[10px] text-white/50 text-center max-w-16 truncate">{step.label}</span>
...
const OrbitVisualization: React.FC<OrbitVisualizationProps> = ({ steps, category }) => {
      ...
      <div className="flex items-center justify-center gap-2 overflow-x-auto py-2">
        {steps.map((step, i) => (
          <React.Fragment key={step.id}>
            <StepCircle step={step} isFirst={i === 0} isLast={i === steps.length - 1} />
            {i < steps.length - 1 && <div className="w-4 h-px bg-white/20 shrink-0" />}
          </React.Fragment>
        ))}
      </div>
    </div>
...
const StreakCounter: React.FC<StreakCounterProps> = ({ streak }) => {
  ...
  return (
    <div className="flex items-center gap-4 text-sm">
      ...
      {streak.bestStreak > 0 && <span className="text-white/40">Best: {streak.bestStreak}</span>}
      <span className="text-white/40">Hit rate: {formatHitRate(streak.hitRate)}</span>
    </div>
  );
};
...
const OrbitSection: React.FC<OrbitSectionProps> = ({
  ...
  return (
    <div className={cn("rounded-xl border overflow-hidden", colors.border, "bg-black/30")}>
      {/* Header */}
      <button
      ...
      >
        ...
        <span className={cn("text-white/30 transition-transform", expanded && "rotate-180")}>
          ▼
        </span>
      </button>
...
const WeeklyAnchorCard: React.FC<WeeklyAnchorCardProps> = ({ template, onClick }) => {
  return (
    <button
      onClick={onClick}
      className={cn(
...
export const HabitsLens: React.FC<HabitsLensProps> = ({
  ...
  const renderSection = (title: string, icon: string, sectionTemplates: HabitTemplate[]) => {
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-white/90">Habits & Rituals</span>
          <span className="text-xs text-white/40">{templates.length} templates</span>
        </div>
        <div className="flex items-center gap-2">
          <button
            className={cn(
        ...
        {weeklyTemplates.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-white/70">Weekly Anchors</span>
            </div>
