...
import { SLEEPTIME_COLORS, STATE_INTENSITIES, STATE_SPEEDS } from "./types";
    ...
    <group ref={rootRef} position={position} scale={scale}>
      ...
      {state !== "dormant" && state !== "complete" && progress > 0 && (
        <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -1.3, 0]}>
          <ringGeometry args={[0.8, 0.85, 64, 1, 0, Math.PI * 2 * Math.min(progress, 1)]} />
          <meshBasicMaterial color={SLEEPTIME_COLORS.rings} transparent opacity={0.8} />
        </mesh>
      )}
    </group>
  );
