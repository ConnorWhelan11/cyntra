...
const sections = [
  {
    title: "Origin Story",
    body: "A candid look at the projects that sparked the Out of Scope universe.",
  },
  {
    title: "Hyperfocus Mode",
    body: "Neurodivergent friendly rituals, timers, and breathing loops.",
  },
    ...
    <ScrollDemo
      ...
      content={
        <div className="pointer-events-none flex min-h-[60vh] flex-col gap-20 px-6 py-28 sm:px-14">
          {["Editorial Moodboards", "Podcast Pipeline", "Personal Experiments"].map(
            (title, idx) => (
              <section key={title} className="flex items-center">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/80 shadow-neon-cyan/30 backdrop-blur">
                  <p className="text-sm text-cyan-200">
                    {idx === 0 ? "Journal" : idx === 1 ? "Audio" : "Play"}
                  </p>
...
export const IntroSequence: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Forces the cinematic intro flow on the ScopeBox within the scene for quick iteration.",
      },
    },
  },
};
