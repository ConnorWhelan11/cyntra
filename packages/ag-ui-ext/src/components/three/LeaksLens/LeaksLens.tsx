...
const LeaksGraphScene: React.FC<LeaksGraphSceneProps> = ({
  ...
  return (
    <>
      ...
      <EffectComposer>
        <Bloom mipmapBlur intensity={0.3} luminanceThreshold={0.7} luminanceSmoothing={0.6} />
      </EffectComposer>
    </>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className={cn("font-medium", severityClasses.text)}>{distraction.label}</div>
...
const DurationPicker: React.FC<DurationPickerProps> = ({ selected, onChange }) => {
...
export const LeaksLens: React.FC<LeaksLensProps> = ({
  ...
  const [localDistractions, setLocalDistractions] = useState<DistractionNode[]>(distractions);
  ...
  const handleLeakAction = useCallback(
    (nodeId: string, action: LeakAction) => {
      setLocalDistractions((prev) => prev.map((d) => (d.nodeId === nodeId ? { ...d, action } : d)));
  ...
  }, [blockedNodeIds, selectedDuration, localDistractions, onSuppressionConfirm]);
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-white/90">Attention Leaks</span>
          <span className="text-xs text-white/40">{localDistractions.length} detected</span>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        <div className="text-xs uppercase tracking-wider text-white/40 mb-3">Detected Leaks</div>
      ...
      {localDistractions.length > 0 && (
        <div className="px-6 py-3 border-t border-white/5">
          <DurationPicker selected={selectedDuration} onChange={handleDurationChange} />
        </div>
      )}
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {localDistractions.length > 0 && (
            <div className="flex gap-3">
              <button
                onClick={() => localDistractions[0] && handleShowWhy(localDistractions[0].nodeId)}
                className={cn(
