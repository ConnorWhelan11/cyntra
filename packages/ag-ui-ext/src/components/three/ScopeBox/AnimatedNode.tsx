...
const NODE_COLOR_MAP = {
  cyan: {
    ...
    labelClass: "text-cyan-100 drop-shadow-[0_0_4px_rgba(31,154,214,0.9)]",
  },
  magenta: {
    sphereHex: "#e879f9", // rare 'hacked' nodes
    labelClass: "text-fuchsia-100 drop-shadow-[0_0_4px_rgba(232,121,249,0.9)]",
  },
  yellow: {
    sphereHex: "#f97316", // villains
    labelClass: "text-amber-200 drop-shadow-[0_0_4px_rgba(251,191,36,0.95)]",
  },
  emerald: {
    sphereHex: "#22c55e", // outcomes
    labelClass: "text-emerald-200 drop-shadow-[0_0_4px_rgba(34,197,94,0.9)]",
  },
};
...
export const AnimatedNode = ({
  ...
  useFrame((state) => {
    ...
    const lerpedPos = new THREE.Vector3().lerpVectors(node.chaosPos, targetPos, progress);
  ...
  return (
    <group ref={meshRef} position={currentPos.current}>
      ...
      >
        <sphereGeometry args={[isHighlighted || isNodeHovered ? 0.14 : 0.07, 16, 16]} />
        <meshBasicMaterial
          color={nodeColor}
          transparent
      ...
      <Html distanceFactor={10} zIndexRange={[0, 0]} portal={htmlPortal}>
        <div
          className={`
        ...
        >
          {node.label}
          {isNodeHovered && <div className="text-[6px] opacity-70 font-normal">{node.type}</div>}
        </div>
      </Html>

      {/* Connection line rendered via NodeLine component for reactive updates */}
      <NodeLine nodeRef={meshRef} color={nodeColor} containmentProgress={containmentProgress} />
    </group>
  );
};
