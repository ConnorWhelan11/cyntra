 ...
 */
function buildSegmentClassName(stepClassName: string | undefined, frame: StoryFrame): string {
  ...
  if (frame.strikethrough) {
    conditionalClasses.push("line-through", "decoration-rose-400/70", "decoration-2");
  }
...
function applyFrame(
  ...
  switch (mode) {
    ...
    case "append-inline": {
      ...
      return [...frozenLines.slice(0, lastLineIndex), [...frozenLines[lastLineIndex], newSegment]];
    }
    ...
    case "mutate-last": {
      ...
      const mutatedSegment = mutateSegment(lastLine[lastSegmentIndex], frame, stepClassName);

      // Return new state with the mutated segment
      return [
        ...frozenLines.slice(0, lastLineIndex),
        [...lastLine.slice(0, lastSegmentIndex), mutatedSegment],
      ];
    }
...
export const MissionIntroPanel = ({
  ...
  useEffect(() => {
    ...
    if (framePhase === "waiting-before") {
      ...
      const timer = setTimeout(() => {
        ...
        setLinesState((prev) => applyFrame(currentFrame, prev, stepClassName, frameIndex));
  ...
  if (frameIndex < 0 || linesState.length === 0) {
    return <div className="w-[140px] sm:w-[160px] md:w-[180px] h-8 pointer-events-none" />;
  }
