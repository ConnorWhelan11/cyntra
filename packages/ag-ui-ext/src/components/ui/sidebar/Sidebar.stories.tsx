...
const AppUsageWrapper = (_props: unknown) => {
  const [open, setOpen] = useState(false);
  const [activeRealmId, setActiveRealmId] = useState("outora");
  const activeRealm = appRealmDefs.find((r) => r.id === activeRealmId) || appRealmDefs[0];
          ...
          <div className="flex flex-1 flex-col overflow-visible">
            {open ? (
              <div className="flex items-center gap-3 py-1 text-xs font-display font-semibold uppercase tracking-[0.35em] text-slate-300">
                ...
                <span className="whitespace-pre font-medium text-white">Out-of-Scope</span>
              </div>
            ) : (
              ...
              <SidebarToolsSlab index={0}>
                {appSidebarGroups.map((group, groupIdx) => (
                  <div key={groupIdx} className="flex flex-col">
                    <SidebarSection title={group.title} accent={group.accent} index={groupIdx} />
