...
const CardContent = ({
  ...
  <>
    ...
    <PixelCanvas colors={pixelColors} gap={pixelGap} speed={pixelSpeed} variant="default" />
    ...
    {/* Content */}
    <div className={cn("relative z-10 flex flex-col", compact ? "gap-2" : "gap-2.5")}>
      {/* Header with icon */}
      <div className="flex items-center justify-between">
        ...
        >
          <span className={compact ? "[&>svg]:h-3.5 [&>svg]:w-3.5" : "[&>svg]:h-4 [&>svg]:w-4"}>
            {link.icon}
          </span>
        </div>
...
export const SidebarFeaturedCard = ({
  ...
  const cardTransition = {
    opacity: {
      ...
      delay: isOpening ? ms(phase3Config.start + 100 + staggerDelay) : ms(staggerDelay * 0.5),
    },
    y: {
      duration: ms(isOpening ? 140 : 80),
      ease: isOpening ? ANIMATION_CONFIG.easing.open : ANIMATION_CONFIG.easing.close,
      delay: isOpening ? ms(phase3Config.start + 90 + staggerDelay) : ms(staggerDelay * 0.5),
    },
    scale: {
