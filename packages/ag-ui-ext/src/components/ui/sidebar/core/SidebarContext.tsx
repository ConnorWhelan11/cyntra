...
import type { AnimationDirection, AnimationPhase, SidebarContextProps } from "../types";
...
export const SidebarProvider = ({
  ...
  const calculatePhase = useCallback(
    (elapsed: number, direction: AnimationDirection): AnimationPhase => {
      ...
      const config = direction === "opening" ? ANIMATION_CONFIG.open : ANIMATION_CONFIG.close;
      ...
      if (direction === "opening") {
        ...
        if (elapsed < config.phase2.end) return "phase2";
        if (elapsed < config.phase3.end) return "phase3";
        if (elapsed < (config as typeof ANIMATION_CONFIG.close).phase4.end) return "phase4";
      }
    ...
    (direction: AnimationDirection) => {
      ...
      const tick = () => {
        ...
        if (phase !== "idle") {
          ...
          if (phaseConfig && typeof phaseConfig === "object" && "start" in phaseConfig) {
