...
export function StudyTimeline({
  ...
  const [visibleItems, setVisibleItems] = React.useState(maxItems || items.length);

  const reducedMotion = prefersReducedMotion();
  const shouldAnimate = !disableAnimations && !reducedMotion;

  const displayedItems = items.slice(0, visibleItems);
  const groupedItems = groupByDate ? groupItemsByDate(displayedItems) : { "": displayedItems };
  ...
  const getItemIcon = (type: TimelineItem["type"], status: TimelineItem["status"]) => {
    if (status === "completed") {
  ...
  const getItemColor = (type: TimelineItem["type"], status: TimelineItem["status"]) => {
    if (status === "completed") {
  ...
  return (
    <div className={cn(studyTimelineVariants({ variant, orientation }), className)} {...props}>
      {orientation === "vertical" ? (
        <div className="relative">
          {/* Timeline line */}
          {showLine && <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-border" />}
      ...
      {showLoadMore && visibleItems < items.length && (
        ...
        >
          <GlowButton
            variant="outline"
            onClick={() => {
              setVisibleItems((prev) => Math.min(prev + (maxItems || 10), items.length));
              onLoadMore?.();
            }}
            disabled={loading}
          >
            {loading ? "Loading..." : `Load More (${items.length - visibleItems} remaining)`}
          </GlowButton>
        </motion.div>
      )}
    </div>
...
function TimelineItemComponent({
  ...
  getItemIcon: (type: TimelineItem["type"], status: TimelineItem["status"]) => React.ReactNode;
  getItemColor: (type: TimelineItem["type"], status: TimelineItem["status"]) => string;
}) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex gap-4",
        orientation === "vertical" ? "items-start" : "flex-col items-center min-w-64"
      )}
      initial={shouldAnimate ? { opacity: 0, y: 20 } : {}}
    ...
    >
      ...
      <motion.div
        className={cn(
          ...
          showLine && orientation === "vertical" && "relative z-10 bg-background"
        )}
        whileHover={shouldAnimate ? { scale: 1.1 } : {}}
        whileTap={shouldAnimate ? { scale: 0.95 } : {}}
        onClick={onClick}
        style={showLine && orientation === "vertical" ? { marginLeft: "-2px" } : {}}
      >
        {getItemIcon(item.type, item.status)}
      </motion.div>

      {/* Content */}
      <div className={cn("flex-1 space-y-2 pb-6", orientation === "vertical" ? "" : "text-center")}>
        {/* Header */}
        ...
        {/* Metadata */}
        {(item.subjects || item.difficulty || item.xpEarned || item.achievements) &&
          showDetails && (
            <AnimatePresence>
              {(!expandable || isExpanded) && (
                ...
                >
                  ...
                  {item.subjects && item.subjects.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {item.subjects.map((subject) => (
                        <StatBadge key={subject} variant="difficulty" value={subject} size="sm" />
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    {item.difficulty && (
                      <StatBadge variant="difficulty" value={item.difficulty} size="sm" />
                    )}
                    {item.xpEarned && (
                      <StatBadge variant="xp" value={item.xpEarned} suffix=" XP" size="sm" />
                    )}
                  </div>
