...
const CalendarDemo = ({ view, showConfirmation, badgeVariant, eventCount }: CalendarStoryProps) => {
  const events = React.useMemo(
    () => CALENDAR_ITEMS_MOCK.slice(0, Math.min(eventCount, CALENDAR_ITEMS_MOCK.length)),
    [eventCount]
  );

  return (
    <div className="w-full max-w-6xl h-[720px]">
      <CalendarProvider events={events} users={USERS_MOCK} view={view} badge={badgeVariant}>
...
const meta: Meta<typeof CalendarDemo> = {
  ...
  argTypes: {
    ...
    showConfirmation: {
      control: "boolean",
      description: "Show a confirmation dialog before dropping an event to a new slot.",
    },
    eventCount: {
...
export const WithDropConfirmation: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Simulates the drag-and-drop confirmation flow when repositioning events.",
      },
    },
  },
};
