    ...
    <div className={cn("flex h-full gap-4", className)}>
      <div className="flex-1 rounded-2xl border border-border/60 bg-card/70 p-4 shadow-sm backdrop-blur">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
            <span className="rounded-full bg-primary/10 px-3 py-1 text-primary">{modeLabel}</span>
      ...
      {showSidebar && (
        <div className="w-full max-w-[360px]">
          <DrawboardSidebar {...sidebar}>{sidebar?.content ?? sidebar?.children}</DrawboardSidebar>
        </div>
      )}
    </div>
  );
