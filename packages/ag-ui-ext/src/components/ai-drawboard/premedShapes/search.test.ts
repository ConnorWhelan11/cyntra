...
describe("searchShapes", () => {
  ...
  it("finds shapes by tag", () => {
    ...
    const hasHeartShape = results.some(
      (r) => r.shape.id === "heart" || r.shape.id === "heart-pulse"
    );
  ...
  it("handles multi-token queries", () => {
    ...
    const hasMatch = results.some(
      (r) =>
        r.shape.name.toLowerCase().includes("test") || r.shape.tags.some((t) => t.includes("blood"))
    );
  ...
  it("sorts by score descending then name alphabetically", () => {
    ...
    for (let i = 1; i < results.length; i++) {
      ...
      if (prevScore === currScore) {
        ...
        expect(results[i - 1].shape.name.localeCompare(results[i].shape.name)).toBeLessThanOrEqual(
          0
        );
      } else {
