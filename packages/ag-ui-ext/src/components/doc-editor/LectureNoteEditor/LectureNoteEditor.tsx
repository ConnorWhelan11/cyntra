    ...
    <div className={cn("lecture-note-editor flex flex-col gap-4", className)}>
      ...
      <div className="flex items-center justify-between border-b border-border/40 pb-3">
        <div>
          ...
          {lectureDate && (
            <p className="text-sm text-muted-foreground">{lectureDate.toLocaleDateString()}</p>
          )}
        </div>
        {tags.length > 0 && (
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          <RichDocEditor {...editorProps} placeholder="Take notes during the lecture..." />
        </div>
        ...
        {professorStreamEnabled && (
          <aside className="w-64 shrink-0 rounded-lg border border-border/40 bg-card/40 p-3">
            <h3 className="mb-2 text-sm font-medium text-muted-foreground">Live Insights</h3>
