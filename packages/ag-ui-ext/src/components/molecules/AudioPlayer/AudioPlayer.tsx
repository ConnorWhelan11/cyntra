...
export const AudioPlayer = ({ title, episodeNumber, className, onClose }: AudioPlayerProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:p-6 flex items-center gap-6">
        {/* Play/Pause Control */}
        <button onClick={() => setIsPlaying(!isPlaying)} className="relative group flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="h-12 flex items-center gap-[2px] opacity-80 overflow-hidden mask-gradient-r">
            {waveformBars.map((bar, i) => (
              <motion.div
                key={i}
                className={cn("w-1 rounded-full", isPlaying ? "bg-cyan-neon" : "bg-neutral-700")}
                initial={{ height: bar.height }}
                animate={{
                  height: isPlaying ? [bar.height, bar.height * 0.5, bar.height] : bar.height,
                }}
                transition={{
                  duration: 0.5,
                  repeat: Infinity,
