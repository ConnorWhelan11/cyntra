    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-white/5">
        <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground">
          <span className={cn("flex items-center gap-1 px-1.5 py-0.5 rounded-sm border bg-black/50", trackColors[track])}>
            {trackIcons[track]}
            {track}
          </span>
        ...
        <div className="flex items-center gap-2">
             ...
             {severity && (
                <span className={cn(
                    ...
                    severity === "S1" ? "bg-red-900/50 text-red-400 border border-red-800" : 
                    severity === "S2" ? "bg-orange-900/50 text-orange-400 border border-orange-800" :
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2 mt-4 pt-2 border-t border-white/5">
          {tags.map(tag => (
            <span key={tag} className="text-[10px] px-2 py-1 rounded-full bg-white/5 text-neutral-400 border border-white/5">
              {tag}
            </span>
          ))}
        </div>
      ...
      {/* Side accent line */}
      <div className={cn("absolute top-0 left-0 w-1 h-full transition-all opacity-50 group-hover:opacity-100", 
         track === 'HUMAN' ? 'bg-cyan-neon shadow-[0_0_10px_var(--cyan-neon)]' :
         track === 'MODEL' ? 'bg-magenta-neon shadow-[0_0_10px_var(--magenta-neon)]' :
