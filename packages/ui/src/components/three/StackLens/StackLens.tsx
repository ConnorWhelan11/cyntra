...
const EnforcementBadge: React.FC<EnforcementBadgeProps> = ({
  state,
  onToggle,
}) => {
      ...
      <div className="flex-1">
        ...
        {state.active && state.suppressedSites && (
          <div className="text-xs text-white/40">
            ...
            {state.suppressedSites.length > 2 &&
              ` +${state.suppressedSites.length - 2}`}
          </div>
        )}
      </div>
      {state.active && (
        <span className="text-xs text-purple-300">{remainingMins}m left</span>
      )}
      {onToggle && (
        <button
...
export const StackLens: React.FC<StackLensProps> = ({
  ...
  const queueTasks = useMemo(
    () =>
      tasks
        .filter((t) => t.id !== activeTask?.id && t.status === "pending")
        .slice(0, 2),
    [tasks, activeTask]
  );
    ...
    if (timerState.status === "running" && timerState.startedAt) {
      timerRef.current = setInterval(() => {
        setTimerState((prev) => {
          const now = Date.now();
          const elapsed =
            Math.floor((now - prev.startedAt!.getTime()) / 1000) +
            (prev.pausedAt ? 0 : 0);
          const newStatus =
            elapsed >= prev.plannedSeconds ? "overtime" : "running";
  ...
  const glyphContext = useMemo(
    () => getStackGlyphContext(tasks, timerState),
    [tasks, timerState]
  );
  ...
  const handleComplete = useCallback(() => {
    if (activeTask) {
      ...
      const remainingTasks = tasks.filter(
        (t) => t.id !== activeTask.id && t.status === "pending"
      );
            ...
            {timerState.status === "idle" ? (
              ...
              >
                ...
                <p className="text-white/40 mb-6">
                  {activeTask.plannedDuration} minutes planned
                </p>
                <button
                  onClick={handleStart}
                  className={cn(
