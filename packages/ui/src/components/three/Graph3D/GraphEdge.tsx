...
export const GraphEdge = ({
  ...
  const isDistraction = edge.type === "distraction" || edge.type === "leak"; // Handle potential type names
  ...
  useFrame((state, delta) => {
    ...
    if (
      lineRef.current &&
      (isAgentPath || isPathActive || isSuggested || isDistraction)
    ) {
      if (lineRef.current.material) {
        // Speed varies by type
        const speed = isPathActive
          ? 1.5
          : isAgentPath
            ? 1
            : isDistraction
