...
export const Graph3D = forwardRef<Graph3DHandle, Graph3DProps>(
  (
    ...
    const [currentLayoutOptions, setCurrentLayoutOptions] =
      useState<LayoutOptions>(layoutOptions);
      ...
      <>
        ...
        <group ref={groupRef}>
          ...
          {graph.edges.map((edge) => {
            ...
            const bothInPath =
              focusedPath.includes(edge.source) &&
              focusedPath.includes(edge.target);
            const isDimmed =
              dimUnhighlighted &&
              !bothInPath &&
              (focusedPath.length > 0 || !!selectedNodeId);
