...
export interface FieldBus {
  ...
  processPendingWithUv: (computeUv: (clientX: number, clientY: number) => { x: number; y: number } | null) => void;
}
...
export function createFieldBus(
  ...
  return {
    ...
    processPendingWithUv(computeUv: (clientX: number, clientY: number) => { x: number; y: number } | null) {
