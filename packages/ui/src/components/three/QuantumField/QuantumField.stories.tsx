...
function DemoSurface({
  ...
  const { onMouseEnter, onMouseMove, onMouseLeave, onClick: handleFieldClick } =
    useFieldSurface({
      id,
      enableHover: true,
      enableBurst: true,
  ...
  return (
    <div className="absolute top-4 right-4 p-3 rounded-lg bg-black/80 border border-slate-700/50 text-xs font-mono text-slate-300 max-w-xs">
      ...
      <div className="space-y-1">
        <div>
          Hover: {state.hover.active ? "✓" : "✗"} | Intent:{" "}
          {state.hover.intent} | UV: ({state.hover.uv.x.toFixed(2)},{" "}
          {state.hover.uv.y.toFixed(2)})
        </div>
    ...
    <div className="relative w-full h-screen bg-slate-950 overflow-hidden">
      ...
      <div className="relative z-10 flex h-full pointer-events-none">
        ...
        <div className="flex-1 p-8 pointer-events-auto">
          ...
          <p className="text-slate-400 mb-8 max-w-lg">
            ...
            Hold <kbd className="px-1.5 py-0.5 rounded bg-slate-700 text-xs">Shift</kbd> while hovering for{" "}
            <span className="text-pink-400">etch</span> mode.
            Click for <span className="text-cyan-300">burst</span> impulses.
            Click a nav item to <span className="text-emerald-400">latch</span> an anchor.
          </p>
      ...
      <div className="absolute bottom-4 left-4 p-3 rounded-lg bg-black/80 border border-slate-700/50 text-xs text-slate-400 max-w-sm">
        ...
        <ul className="space-y-0.5">
          <li>• <span className="text-cyan-400">Hover</span> = probe (reveal)</li>
          <li>• <span className="text-pink-400">Shift + Hover</span> = etch (write trails)</li>
          <li>• <span className="text-cyan-300">Click</span> = burst (impulse ring)</li>
          <li>• <span className="text-emerald-400">Click nav</span> = latch (anchor)</li>
        </ul>
      </div>
    </div>
  );
...
export const PerformanceLevels: Story = {
  render: () => {
    const [level, setLevel] = useState<"high" | "medium" | "low" | "minimal">(
      "high"
    );
        ...
        <div className="relative w-full h-screen bg-slate-950 overflow-hidden">
          ...
          <div className="relative z-20 p-4 pointer-events-auto max-w-sm max-h-[calc(100vh-2rem)] overflow-y-auto">
            ...
            <div className="mb-4 p-2 rounded bg-black/60 text-xs text-slate-400">
              <p className="mb-1"><span className="text-cyan-400">Hover</span> = phase lens + reveal lattice</p>
              <p className="mb-1"><span className="text-orange-400">Shift+Hover</span> = etch/inscribe</p>
              <p className="mb-1"><span className="text-emerald-400">Scribble Mode</span> = pointer down draws</p>
              <p className="mt-2 text-purple-400 font-medium">🔮 Try Orchid palette for museum-grade vibes!</p>
            ...
            <div className="space-y-3 pr-1">
              ...
              <ControlPanel title="Visibility">
                ...
                <SelectControl 
                  ...
                  options={[{ value: "normal", label: "Normal" }, { value: "additive", label: "Additive" }]}
                />
              </ControlPanel>
              ...
              <ControlPanel title="Micro-Lattice">
                ...
                />
                <Slider label="Grid 1" value={microGrid1} onChange={setMicroGrid1} min={20} max={100} step={5} />
                <Slider label="Grid 2" value={microGrid2} onChange={setMicroGrid2} min={100} max={400} step={20} />
                <Slider label="Strength" value={microGridStrength} onChange={setMicroGridStrength} min={0} max={1} step={0.05} />
                <Slider label="Reveal" value={revealStrength} onChange={setRevealStrength} min={0} max={1} step={0.05} />
                <Slider label="Warp" value={microWarp} onChange={setMicroWarp} min={0} max={0.05} step={0.005} />
              </ControlPanel>

              {/* Phase Lens Panel */}
              <ControlPanel title="Phase Lens ✨">
                <Toggle label="Enabled" checked={lensEnabled} onChange={setLensEnabled} />
                <Slider label="Radius" value={lensRadius} onChange={setLensRadius} min={0.05} max={0.25} step={0.01} />
                <Slider label="Magnify" value={lensMagnification} onChange={setLensMagnification} min={0.5} max={2.0} step={0.1} />
                <Slider label="Chromatic" value={lensChromatic} onChange={setLensChromatic} min={0} max={1} step={0.05} />
                <Slider label="Inertia" value={lensInertia} onChange={setLensInertia} min={0.05} max={0.5} step={0.02} />
                <Slider label="Vel Boost" value={lensVelocityBoost} onChange={setLensVelocityBoost} min={0} max={1} step={0.1} />
              </ControlPanel>

              {/* Etch Panel */}
              <ControlPanel title="Etch / Inscription">
                <Slider label="Base Radius" value={etchRadius} onChange={setEtchRadius} min={0.002} max={0.03} step={0.002} />
                <Slider label="Strength" value={etchStrength} onChange={setEtchStrength} min={0.1} max={1} step={0.05} />
                <Slider label="Decay" value={etchDecay} onChange={setEtchDecay} min={0} max={0.1} step={0.005} />
                <Slider label="Distort" value={etchDistortion} onChange={setEtchDistortion} min={0} max={0.02} step={0.002} />
                ...
                <div className="mt-2 pt-2 border-t border-slate-700/50">
                  ...
                  <Slider label="Min Radius" value={etchRadiusMin} onChange={setEtchRadiusMin} min={0.001} max={0.01} step={0.001} />
                  <Slider label="Max Radius" value={etchRadiusMax} onChange={setEtchRadiusMax} min={0.01} max={0.03} step={0.002} />
                  <Slider label="Vel Scale" value={etchVelocityScale} onChange={setEtchVelocityScale} min={0} max={1} step={0.1} />
                </div>
                <button
              ...
              <ControlPanel title="Palette / Atmosphere ✨">
                ...
                />
                <Slider label="Accent" value={accentIntensity} onChange={setAccentIntensity} min={0} max={2} step={0.1} />
                <div className="mt-2 pt-2 border-t border-slate-700/50">
                  <div className="text-[10px] text-slate-500 mb-1">Iridescence</div>
                  <Slider label="Strength" value={iridescenceStrength} onChange={setIridescenceStrength} min={0} max={1} step={0.05} />
                  <Slider label="Scale" value={iridescenceScale} onChange={setIridescenceScale} min={1} max={40} step={1} />
                </div>
                <div className="mt-2 pt-2 border-t border-slate-700/50">
                  <div className="text-[10px] text-slate-500 mb-1">Atmosphere</div>
                  <Slider label="Exposure" value={exposure} onChange={setExposure} min={0.6} max={2} step={0.05} />
                  <Slider label="Filmic" value={filmic} onChange={setFilmic} min={0} max={1} step={0.05} />
                  <Slider label="Grain" value={grainStrength} onChange={setGrainStrength} min={0} max={0.08} step={0.005} />
                  <Slider label="CRT" value={crtStrength} onChange={setCrtStrength} min={0} max={1} step={0.05} />
                  <Slider label="Copper" value={copperStrength} onChange={setCopperStrength} min={0} max={1} step={0.05} />
                </div>
              </ControlPanel>
