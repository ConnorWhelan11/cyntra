...
const questionCardVariants = cva(
  "relative w-full rounded-lg border bg-card/40 backdrop-blur-sm transition-all duration-300 overflow-hidden",
  {
    variants: {
      state: {
        default:
          "border-border/40 hover:border-cyan-neon/40 hover:shadow-neon-cyan/20",
        selected: "border-cyan-neon/60 bg-cyan-neon/5 shadow-neon-cyan/30",
        correct:
          "border-emerald-neon/60 bg-emerald-neon/5 shadow-neon-emerald/30",
        incorrect:
    ...
    if (choiceState === "correct")
      return (
        <CheckCircle2
          className={cn(
            "h-4 w-4",
            isCalm ? "text-emerald-600" : "text-emerald-neon"
          )}
        />
      );
    if (choiceState === "incorrect" && choiceId === selectedAnswerId) {
      return (
        <XCircle
          className={cn(
            "h-4 w-4",
            isCalm ? "text-red-600" : "text-destructive"
          )}
        />
      );
    }
    ...
    >
      ...
      {shouldAnimate && state === "correct" && (
        <motion.div
          className={cn(
            ...
            isCalm
              ? "from-emerald-500/5 via-emerald-500/2"
              : "from-emerald-neon/10 via-cyan-neon/5"
          )}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      {shouldAnimate && state === "incorrect" && (
        <motion.div
          className={cn(
            "absolute inset-0 bg-gradient-to-br to-transparent",
            isCalm
              ? "from-red-500/5 via-red-500/2"
              : "from-destructive/10 via-red-500/5"
          )}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5 }}
      ...
      <div className="relative z-10 flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          {/* Subject badge */}
          {subject && (
            <StatBadge variant="difficulty" value={subject} size="sm" />
          )}
          ...
          >
            ...
            <motion.div
              className={cn(
                ...
                state === "correct" &&
                  "bg-emerald-neon/10 border border-emerald-neon/20",
                state === "incorrect" &&
            ...
            >
              {state === "correct" ? (
                <>
                  ...
                  <span className="text-sm font-medium text-emerald-neon">
                    Correct! Well done.
                  </span>
                </>
              ) : state === "incorrect" ? (
