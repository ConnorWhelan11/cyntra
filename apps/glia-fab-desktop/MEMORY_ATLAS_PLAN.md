...
- **Layers + spacing**: `apps/glia-fab-desktop/src/features/memory/lifecycle/strata.ts`
  - Vault is anchored at `y=0` in both modes.
  - `Vault View` uses tight spacing; `Lifecycle View` spreads layers (camera reframes).
- **Type → sigil**: `apps/glia-fab-desktop/src/features/memory/lifecycle/mappings.ts`
...
```typescript
function computeNodePosition(memory: MemoryItem, index: number, total: number): [number, number, number] {
