...
import type { GameplayTab, EntityConfig, ObjectiveConfig, TriggerConfig, RuntimeState } from "@/types";
...
export function GameplayView({ worldPath, onNavigate }: GameplayViewProps) {
  ...
  const npcs = config
    ? Object.entries(config.entities).filter(([_, e]) => e.type === "npc")
    : [];
  const items = config
    ...
    <div className="gameplay-view">
      ...
      <div className="gameplay-toolbar">
        <div className="gameplay-toolbar-left">
          ...
          <span className="gameplay-title">
            {config?.world_id || "Gameplay Editor"}
          </span>
        ...
        <div className="gameplay-toolbar-actions">
          ...
          <button className="gameplay-action-btn gameplay-action-btn--secondary" onClick={handleExport}>
            Export JSON
          </button>
        </div>
      </div>
          ...
          <div className="gameplay-tabs">
            {tabs.map((tab) => (
              ...
              >
                {tab.label}
                {tab.count !== undefined && (
                  <span className="gameplay-tab-count">{tab.count}</span>
                )}
              </button>
            ))}
          </div>
...
function EntitiesPanel({
  ...
  const getEntityStatus = (id: string, type: string) => {
    ...
    const matchedList = type === "npc" ? validationReport.matched_npcs : validationReport.matched_items;
    if (matchedList?.includes(id)) return "valid";
    if (validationReport.missing_markers?.some((m: any) => m.entity_id === id)) return "invalid";
    return "unknown";
  };
...
function AudioZonesPanel({ audioZones }: { audioZones: [string, any][] }) {
  return (
    <div>
      <div className="gameplay-entity-list">
...
function ValidationPanel({ validationReport }: { validationReport: any }) {
  if (!validationReport) {
    return (
    ...
    <div>
      ...
      {validationReport.missing_markers?.length > 0 && (
        <div className="gameplay-validation-section">
          ...
          <div className="gameplay-validation-list">
            {validationReport.missing_markers.map((issue: any, i: number) => (
              <div key={i} className="gameplay-validation-item gameplay-validation-item--error">
                <span>✗</span>
                <div>
                  <div>{issue.entity_id}</div>
                  <div className="gameplay-validation-detail">Expected: {issue.expected_marker}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {validationReport.warnings?.length > 0 && (
        <div className="gameplay-validation-section">
          ...
          <div className="gameplay-validation-list">
            {validationReport.warnings.map((warning: any, i: number) => (
              <div key={i} className="gameplay-validation-item gameplay-validation-item--warning">
                <span>⚠</span>
  ...
  return (
    <div>
      ...
      <div className="gameplay-inspector-field">
        ...
        />
        {actionsError && (
          <div className="gameplay-inspector-error">{actionsError}</div>
        )}
      </div>
...
function ObjectiveInspector({
  ...
  const requiresPlaceholder =
    allObjectives.length > 0
      ? allObjectives.map((o) => o.id).slice(0, 3).join(", ")
    ...
    <div>
      ...
      <div className="gameplay-inspector-field">
        ...
        <select
          ...
          onChange={(e) => onUpdate({ ...objective, type: e.target.value as ObjectiveConfig["type"] })}
        >
