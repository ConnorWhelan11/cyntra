...
import type { GameplayConfig, ValidationReport, RuntimeState, ObjectiveStatus } from "@/types";
...
export function GameplayOverlay({
  ...
  const completedCount = Object.values(objectiveStates).filter(
    (s) => s === "completed"
  ).length;
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="gameplay-overlay-tabs">
            <button
              className={`gameplay-overlay-tab ${
                activeSection === "objectives" ? "active" : ""
              }`}
          ...
          <div className="gameplay-overlay-content">
            ...
            {activeSection === "flags" && (
              <FlagsSection flags={flags} activeFlags={activeFlags} />
            )}
          ...
          <div className="gameplay-overlay-footer">
            {validation ? (
              <div
                className={`gameplay-overlay-validation ${
                  validation.valid ? "valid" : "invalid"
                }`}
              >
                {validation.valid ? "✓ Valid" : `⚠ ${validation.missing_markers?.length || 0} issues`}
              </div>
            ) : (
...
function EntitiesSection({
  ...
  const npcs = entities.filter(([_, e]) => e.type === "npc");
  const items = entities.filter(([_, e]) =>
  ...
  return (
    <div>
      ...
      <div className="gameplay-section">
        ...
        <div className="gameplay-item-list">
          {items.map(([id, entity]: [string, any]) => (
            <div key={id} className="gameplay-item">
              <div
                className={`gameplay-item-status ${
