...
describe("RefinementQueue", () => {
  ...
  describe("Empty State", () => {
    it("should render nothing when no refinements", () => {
      const { container } = render(
        <RefinementQueue refinements={[]} onApplyNow={vi.fn()} />
      );
  ...
  describe("Rendering", () => {
    ...
    it("should truncate long refinement text", () => {
      ...
      expect(screen.getByText("This is a very long refinement text that should be...")).toBeInTheDocument();
    });
    ...
    it("should show full text in title attribute", () => {
      ...
      const textElement = screen.getByTitle("This is a very long refinement text that should be truncated");
  ...
  describe("Apply Now Button", () => {
    ...
    it("should call onApplyNow with refinement id", async () => {
      ...
      render(<RefinementQueue refinements={queuedRefinements} onApplyNow={defaultProps.onApplyNow} />);
  ...
  describe("Multiple Refinements", () => {
    ...
    it("should call onApplyNow with correct id for each button", async () => {
      const user = userEvent.setup();
      const multipleQueued: RefinementMessage[] = [
        { id: "ref-1", text: "First queued", timestamp: Date.now(), status: "queued", issueId: "issue-1" },
        { id: "ref-2", text: "Second queued", timestamp: Date.now(), status: "queued", issueId: "issue-2" },
      ];
