...
vi.mock("./AgentPanel", () => ({
  AgentPanel: ({ agents, isSpeculating }: any) => (
    <div data-testid="agent-panel" data-agent-count={agents.length} data-speculating={isSpeculating}>
      Agent Panel
    </div>
  ),
}));
...
describe("BuildingConsole", () => {
  ...
  describe("Rendering", () => {
    ...
    it("should display truncated prompt for long prompts", () => {
      const longPromptState = {
        ...mockBuildStateGenerating,
        prompt: "This is a very long prompt that exceeds 80 characters and should be truncated with an ellipsis at the end for display purposes",
      };
      render(<BuildingConsole {...defaultProps} buildState={longPromptState} />);

      // The prompt is truncated to 80 chars - actual text is "wi..."
      expect(screen.getByText(/This is a very long prompt that exceeds 80 characters and should be truncated wi\.\.\./)).toBeInTheDocument();
    });
  ...
  describe("Control Buttons - Paused State", () => {
    it("should show Resume button when paused", () => {
      render(
        <BuildingConsole {...defaultProps} buildState={mockBuildStatePaused} />
      );
  ...
  describe("Control Buttons - Complete State", () => {
    it("should show View in Evolution button when complete", () => {
      render(
        <BuildingConsole {...defaultProps} buildState={mockBuildStateComplete} />
      );
  ...
  describe("Control Buttons - Failed State", () => {
    it("should show Retry button when failed", () => {
      render(
        <BuildingConsole {...defaultProps} buildState={mockBuildStateFailed} />
      );
  ...
  describe("Preview Mode Toggle", () => {
    ...
    it("should call onPreviewModeChange when toggling to asset", async () => {
      ...
      render(
        <BuildingConsole
          {...defaultProps}
          buildState={mockBuildStateComplete}
          previewMode="game"
        />
      );
  ...
  describe("Child Components", () => {
    it("should pass agents to AgentPanel", () => {
      render(<BuildingConsole {...defaultProps} />);

      const agentPanel = screen.getByTestId("agent-panel");
      expect(agentPanel).toHaveAttribute(
        "data-agent-count",
        String(mockBuildStateGenerating.agents.length)
      );
    });

    it("should pass speculation status to AgentPanel", () => {
      render(
        <BuildingConsole {...defaultProps} buildState={mockBuildStateSpeculating} />
      );

      const agentPanel = screen.getByTestId("agent-panel");
      expect(agentPanel).toHaveAttribute("data-speculating", "true");
    });

    it("should pass preview mode to WorldPreview", () => {
      render(<BuildingConsole {...defaultProps} previewMode="asset" />);

      const preview = screen.getByTestId("world-preview");
      expect(preview).toHaveAttribute("data-mode", "asset");
    });
