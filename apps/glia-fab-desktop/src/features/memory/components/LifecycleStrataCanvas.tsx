...
function AmbientEnvironment() {
  return (
    <>
      ...
      <pointLight position={[-10, -6, -10]} intensity={0.35} color="#64d2c8" distance={35} decay={2} />
...
function StrataSlab({ label, position, isFocus = false }: StrataSlabProps) {
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {/* Etched label */}
      <Html position={[-slabWidth / 2 + 0.8, slabThickness / 2 + 0.02, slabDepth / 2 - 0.6]} transform>
        <div
        ...
        >
          <div className="text-[10px] uppercase tracking-[0.22em] text-tertiary font-mono">{label}</div>
        </div>
      </Html>
    </group>
  );
...
function VaultTile({ memory, position, isSelected, isHovered, isFiltered, onSelect, onHover }: VaultTileProps) {
  const groupRef = useRef<THREE.Group>(null);
...
function PlaybackSimulation({
  ...
  const extractionY = getLifecycleLayerY(Math.max(0, extractionIndex), view) + slabThickness / 2 + 0.2;
  ...
  useFrame((state) => {
    ...
    sim.shards.forEach((s, i) => {
      ...
      const extractionPos = new THREE.Vector3(-2.4 + s.offset.x * 0.55, extractionY, s.offset.z * 0.65);
  ...
  return (
    <group>
      ...
      {/* Extracted shards */}
      {sim.shards.map((s, i) => (
        <mesh key={s.shard.temp_id} ref={(el) => { shardRefs.current[i] = el; }}>
          <Sigil type={s.type} />
          <meshStandardMaterial
            color={s.color}
            emissive={s.color}
      ...
      {/* Merged output runes */}
      {sim.groups.map((g, i) => (
        <mesh key={g.key} ref={(el) => { groupRefs.current[i] = el; }}>
          <Sigil type={g.type} />
          <meshStandardMaterial
            color={g.color}
            emissive={g.color}
...
export function LifecycleStrataCanvas({ className = "", dataset }: LifecycleStrataCanvasProps) {
  ...
  const filaments = useMemo(() => {
    return visibleEdges
      .map((e) => {
        ...
        return { key: `${e.a}:${e.link_type}:${e.b}`, points: [fromV, mid, toV] as const, color, arrowPos, arrowQuat };
      })
  ...
  useEffect(() => {
    ...
    const runId =
      (selectedRunId && dataset.runs.some((r) => r.run_id === selectedRunId) ? selectedRunId : null) ??
      dataset.runs
        .slice()
    ...
    const typeOrder: MemoryType[] = ["pattern", "failure", "dynamic", "context", "playbook", "frontier"];
    const groupedTypes = [...byType.keys()].sort((a, b) => typeOrder.indexOf(a) - typeOrder.indexOf(b));
    ...
    const groups: PlaybackSimGroup[] = groupedTypes.map((type, i) => {
      ...
      const avgImportance = clamp01(
        shardIndexes.reduce((sum, si) => sum + shardsForRun[si].shard.provisional_importance, 0) / Math.max(1, shardIndexes.length)
      );
  ...
  }, [dataset, setLifecycleView, setPlaybackRunning, state.playback.nonce]);
        ...
        {originRun && (
          <group>
            {(() => {
              const runsIndex = LIFECYCLE_LAYERS.findIndex((l) => l.id === "runs");
              const slabThickness = 0.08;
              const runsY = getLifecycleLayerY(Math.max(0, runsIndex), state.lifecycleView) + slabThickness / 2 + 0.22;
              ...
              return (
                <>
                  <Line points={[fromV, mid, marker]} color="#7eb8da" transparent opacity={0.22} lineWidth={1} />
                  ...
                  <Html position={[-3.6, runsY + 0.28, 0]} center style={{ pointerEvents: "none" }} transform>
                    <div
                      className="px-2 py-1 rounded-md border border-white/10"
