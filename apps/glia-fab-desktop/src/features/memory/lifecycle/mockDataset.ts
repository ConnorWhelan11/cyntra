...
const TYPE_POOL: LifecycleMemoryType[] = ["pattern", "failure", "dynamic", "context", "playbook", "frontier"];
...
export function createMockLifecycleAtlasMemories(seed: string): { dataset: MockLifecycleDataset; memories: MemoryItem[] } {
  ...
  dataset.links.forEach((l) => {
    ...
    if (!target) return;
    const next: MemoryLink = { type: l.link_type, targetId: l.b, targetTitle: shortTitle(target.text) };
