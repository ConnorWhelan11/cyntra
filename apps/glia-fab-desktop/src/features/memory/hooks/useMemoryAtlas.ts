...
const TYPE_ORDER: MemoryType[] = ["pattern", "failure", "dynamic", "context", "playbook", "frontier"];
...
export function useMemoryAtlas(memories: MemoryItem[], options: UseMemoryAtlasOptions = {}): MemoryAtlasContextValue {
  ...
  const flyToMemory = useCallback((id: string) => {
    const pos = nodePositions.get(id);
    if (pos) {
      setCamera((prev) => ({ ...prev, target: pos, isAnimating: true }));
    }
  ...
  const setLifecycleView = useCallback((view: LifecycleViewMode) => {
    setLifecycleViewState(view);
    if (layout !== "lifecycle") return;

    const fallbackTarget: [number, number, number] = [0, 0, 0];
    const target = (selectedMemoryId ? nodePositions.get(selectedMemoryId) : null) ?? fallbackTarget;

    const offset: [number, number, number] =
      view === "vault" ? [0, 3.5, 10] : [0, 6.5, 15.5];
    setCamera({
      target,
      position: [target[0] + offset[0], target[1] + offset[1], target[2] + offset[2]],
      isAnimating: true,
