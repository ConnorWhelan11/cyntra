...
const TYPE_CONFIG: Record<string, { color: string; icon: string; label: string; desc: string }> = {
  pattern: { color: "var(--accent-primary)", icon: "◈", label: "Pattern", desc: "Learned behavior or best practice" },
  failure: { color: "var(--signal-error)", icon: "⚠", label: "Failure", desc: "Recorded error or anti-pattern" },
  dynamic: { color: "var(--signal-active)", icon: "◎", label: "Dynamic", desc: "Contextual or evolving knowledge" },
  context: { color: "var(--signal-info)", icon: "◐", label: "Context", desc: "Static project or codebase info" },
};
...
export function MemoryDetail({
    ...
    return (
      <div className={`h-full flex flex-col items-center justify-center p-8 ${className}`}>
        <div className="relative mb-6">
          ...
          {/* Decorative rings */}
          <div className="absolute inset-0 rounded-2xl border border-dashed border-slate/20 -m-2 animate-spin" style={{ animationDuration: "20s" }} />
          <div className="absolute inset-0 rounded-2xl border border-dashed border-slate/10 -m-4 animate-spin" style={{ animationDuration: "30s", animationDirection: "reverse" }} />
        </div>
  ...
  const agentConfig = AGENT_CONFIG[memory.agent] || { color: "var(--text-tertiary)", label: memory.agent };
      ...
      <div className="flex-1 overflow-auto p-4 space-y-5">
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-mono tracking-widest text-tertiary uppercase">Content</span>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-mono tracking-widest text-tertiary uppercase">Metadata</span>
            <div className="flex-1 h-px bg-gradient-to-r from-slate to-transparent" />
          </div>
