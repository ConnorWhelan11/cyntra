...
export function MemoryFilters({
  ...
  return (
    <div className={`p-4 space-y-6 ${className}`}>
      ...
      <section>
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs font-mono tracking-widest text-tertiary uppercase">Importance</span>
          <div className="flex-1 h-px bg-gradient-to-r from-slate to-transparent" />
        </div>
      ...
      <div className="pt-4 border-t border-slate/30">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-mono text-secondary">
            {selectedTypes.length + (selectedScope !== "all" ? 1 : 0) + selectedAgents.length + (importanceThreshold > 0 ? 1 : 0)}
          </span>
        </div>
      </div>
    </div>
