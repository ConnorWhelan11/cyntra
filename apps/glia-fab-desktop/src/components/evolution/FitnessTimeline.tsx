...
export function FitnessTimeline({
  ...
  const chartData = useMemo(() => {
    ...
    const scaleY = (fit: number) =>
      padding.top + chartHeight - (fit / maxFit) * chartHeight;

    // Generate path
    const sortedData = [...data].sort((a, b) => a.generation - b.generation);
    const pathPoints = sortedData.map(
      (d) => `${scaleX(d.generation)},${scaleY(d.fitness)}`
    );
    ...
    const currentX =
      currentGeneration !== undefined ? scaleX(currentGeneration) : null;
