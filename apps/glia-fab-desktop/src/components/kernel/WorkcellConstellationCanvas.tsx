...
import type { ConstellationNode, ConstellationEdge, ConstellationStateReturn } from "./useConstellationState";
...
function ConstellationEdgeLine({
  edge,
  positions,
  isHighlighted,
}: ConstellationEdgeLineProps) {
  ...
  const { color, opacity, lineWidth, dashed } = useMemo(() => {
    ...
    if (isHighlighted) {
      return { ...style, opacity: Math.min(style.opacity * 2.5, 1), lineWidth: style.lineWidth + 1 };
    }
    return style;
