  ...
  syncLegacyRecentWorlds(): void {
    try {
      ...
      for (const w of worlds) {
        ...
        const id = typeof world.id === "string" && world.id.trim().length > 0 ? world.id : generateId();
        const title = typeof world.name === "string" && world.name.trim().length > 0 ? world.name : "World";
