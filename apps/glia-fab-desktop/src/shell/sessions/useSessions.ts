...
export function useSessions(filter?: SessionFilter) {
  // Memoize the filter key to avoid recreating getSnapshot on every render
  const filterKey = useMemo(
    () =>
      filter
        ? `${filter.appId ?? ""}_${filter.pinned ?? ""}_${filter.archived ?? ""}_${filter.status ?? ""}`
        : "__all__",
    [filter?.appId, filter?.pinned, filter?.archived, filter?.status]
  );

  // getSnapshot must return the same reference for the same state
  const getSnapshot = useCallback(
    () => sessionStore.getSessions(filter),
    [filterKey]
  );
...
export function useSessionActions() {
  const createSession = useCallback(
    (appId: AppId, title?: string, data?: unknown): Session => {
  ...
  const updateSession = useCallback(
    (id: string, updates: Partial<Session>): void => {
