...
describe('useKernelEventStream', () => {
  ...
  it('should filter events by projectRoot', async () => {
    ...
    emit('kernel_events', {
      ...
      events: [{ timestamp: '2024-01-01T00:00:00Z', type: 'test', issueId: null, workcellId: null, data: null }],
      offset: 100,
    });
    ...
    emit('kernel_events', {
      ...
      events: [{ timestamp: '2024-01-01T00:00:01Z', type: 'test', issueId: null, workcellId: null, data: null }],
      offset: 200,
    });
  ...
  it('should stop event watcher on unmount', async () => {
    ...
    const { unmount } = renderHook(() =>
      useKernelEventStream('/path/to/project', onEvents)
    );
...
describe('useEventStreamCallback', () => {
  it('should return stable callback reference', () => {
    ...
    const { result, rerender } = renderHook(() =>
      useEventStreamCallback(callback)
    );
