    ...
    <div className={`game-console ${isOpen ? "open" : "collapsed"}`}>
      <div className="game-console-header">
        <button className="game-console-toggle" onClick={onToggle}>
          <span className="game-console-toggle-icon">{isOpen ? "▼" : "▲"}</span>
          <span className="game-console-title">Console</span>
          {!isOpen && (errorCount > 0 || warnCount > 0) && (
            <span className="game-console-badges">
              {errorCount > 0 && <span className="game-console-badge error">{errorCount}</span>}
              {warnCount > 0 && <span className="game-console-badge warn">{warnCount}</span>}
            </span>
          )}
        </button>
      ...
      {isOpen && (
        <div ref={scrollRef} className="game-console-logs" onScroll={handleScroll}>
          {logs.length === 0 ? (
            <div className="game-console-empty">No logs yet. Start the game to see output.</div>
          ) : (
            logs.map((entry) => (
              <div key={entry.id} className={`console-entry ${entry.level}`}>
                ...
                <span className="console-entry-icon">{getLevelIcon(entry.level)}</span>
                <span className="console-entry-message">{entry.message}</span>
                {entry.source && <span className="console-entry-source">{entry.source}</span>}
              </div>
            ))
          )}
        </div>
