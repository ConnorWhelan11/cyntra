...
export const StageLayout = memo(function StageLayout({
  ...
  return (
    <div
      className={`stage-layout ${consoleOpen ? "console-open" : ""} ${hasRightPanel ? "has-right-panel" : ""}`}
    >
      ...
      <div className="stage-layout-main">
        ...
        <div className={`stage-layout-console ${consoleOpen ? "open" : ""}`}>{console}</div>
      </div>
      {hasRightPanel && <aside className="stage-layout-right">{rightPanel}</aside>}
    </div>
  );
});
