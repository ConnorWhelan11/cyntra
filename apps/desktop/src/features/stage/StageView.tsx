...
import { StageLayout, WorldSelector, GameFrame, GameConsole, StageControls } from "./components";
...
export function StageView({
  ...
  const { addLog, selectWorld, selectedWorld, selectedWorldId, worlds } = state;
  ...
  useEffect(() => {
    ...
    if (selectedWorldId === initialWorldId) return;
    if (!worlds.some((w) => w.id === initialWorldId)) return;
    selectWorld(initialWorldId);
  }, [initialWorldId, selectWorld, selectedWorldId, worlds]);
  ...
  useEffect(() => {
    if (selectedWorld) {
      addLog("info", `Selected world: ${selectedWorld.name}`, "stage");
    }
  }, [addLog, selectedWorld]);
