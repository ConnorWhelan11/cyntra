      ...
      switch (message.type) {
        ...
        case "save_presentation": {
          ...
          if (payload?.id && payload?.data) {
            try {
              await ImmersaService.savePresentation(activeProject.root, payload.id, payload.data);
    ...
    async (overrideText?: string) => {
      ...
      if (lower === "watch") {
        ...
        try {
          ...
          addAgentChat("system", job ? `Watch started. (run ${job.runId})` : "Watch started.");
        } catch (e) {
        ...
        <div style={{ padding: 8, overflowY: "auto", flex: 1 }}>
          {presentations.length === 0 ? (
            <div className="text-muted-foreground" style={{ padding: 8 }}>
              ...
              {isReady ? " Create one in Immersa." : " Waiting for Immersa to load..."}
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {presentations.map((id) => (
                ...
                >
                  ...
                  <Button variant="ghost" size="small" onClick={() => handleDeletePresentation(id)}>
                    ×
                  </Button>
                </div>
              ))}
      ...
      <Panel style={{ width: 360, flexShrink: 0 }}>
        ...
        <div style={{ padding: 10, borderBottom: "1px solid var(--border)" }}>
          <div className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>
            ...
            <div>
              Limit: {deckLimit ?? "none"} · Outora: {includeOutora ? "on" : "off"} · Runs:{" "}
              {includeRuns ? "on" : "off"}
            </div>
            <div>
            ...
            <div>Watch: {watchJobId ? <span>on ({watchRunId ?? watchJobId})</span> : "off"}</div>
          </div>
          <div style={{ height: 10 }} />
          <div className="row" style={{ gap: 8 }}>
            ...
            <Button
              variant="ghost"
              onClick={() =>
                sendAgentChat(watchJobId ? "stop" : "watch").catch((e) => setError(String(e)))
              }
              disabled={!activeProject || agentBusy}
            >
        ...
        <div className="chat" style={{ padding: 10, height: "100%" }}>
          <div className="chat-log" style={{ height: "calc(100% - 44px)" }}>
            {agentMessages.length === 0 && (
              <div className="muted">
                Try:{" "}
                <code>make a deck called &ldquo;Cyntra Latest&rdquo; with 12 slides and watch</code>{" "}
                then <code>generate</code> and <code>load</code>.
              </div>
            )}
            {agentMessages.slice(-60).map((m) => (
              <div key={m.id} className={"chat-msg " + (m.role === "user" ? "user" : "system")}>
