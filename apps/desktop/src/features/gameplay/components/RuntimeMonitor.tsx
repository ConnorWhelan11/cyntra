...
export function RuntimeMonitor({
  ...
  const sortedFlags = useMemo(() => {
    ...
    return Object.entries(runtimeState.flags).sort(([a], [b]) => a.localeCompare(b));
  }, [runtimeState?.flags]);
    ...
    <div className={`runtime-monitor ${isExpanded ? "expanded" : "collapsed"}`}>
      ...
      <div className="runtime-monitor-header" onClick={toggleExpanded}>
        <div className="runtime-monitor-header-left">
          ...
          <span className={`runtime-monitor-status ${isConnected ? "connected" : "disconnected"}`}>
                ...
                <div className="runtime-objectives-list">
                  {objectives.map((obj) => {
                    ...
                    return (
                      ...
                      >
                        <span className="runtime-objective-icon">{getStatusIcon(status)}</span>
                        <span className="runtime-objective-id">{obj.id}</span>
                        <span
                          className={`runtime-objective-type runtime-objective-type--${obj.type}`}
                        >
                          {obj.type}
                        </span>
                      </div>
              ...
              {recentTriggers.length === 0 ? (
                ...
                <div className="runtime-triggers-list">
                  {recentTriggers.map((event) => (
                    <div
                      key={`${event.trigger_id}-${event.timestamp}`}
                      className="runtime-trigger-event"
                    >
                      <span className="runtime-trigger-time">{formatTime(event.timestamp)}</span>
                      <span className="runtime-trigger-id">{event.trigger_id}</span>
                      <span className="runtime-trigger-action">
                        {event.actions_executed?.length
                          ? `${event.actions_executed.length} actions`
                          : "activated"}
                      </span>
                    </div>
                  ))}
              ...
              {sortedFlags.length === 0 ? (
                ...
                <div className="runtime-flags-grid">
                  {sortedFlags.map(([key, value]) => (
                    <div key={key} className={`runtime-flag ${value ? "active" : "inactive"}`}>
