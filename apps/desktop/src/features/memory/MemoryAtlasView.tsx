...
export function MemoryAtlasView({ activeProject }: MemoryAtlasViewProps) {
  ...
  const atlasContext = useMemoryAtlas(memories, {
    layout: scene === "atlas" ? "atlas" : "lifecycle",
  });
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case "/":
          e.preventDefault();
          document
            .querySelector<HTMLInputElement>('input[placeholder="Search memories..."]')
            ?.focus();
          break;
        case "r":
        case "R":
      ...
      <div className="h-full flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="relative mx-auto w-32 h-32">
            ...
            <div
              className="absolute inset-0 rounded-full border border-dashed border-slate/20 animate-spin"
              style={{ animationDuration: "20s" }}
            />
            <div
              className="absolute inset-4 rounded-full border border-dashed border-slate/30 animate-spin"
              style={{ animationDuration: "15s", animationDirection: "reverse" }}
            />
            <div
              className="absolute inset-8 rounded-full border border-dashed border-accent-primary/20 animate-spin"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-accent-primary/20 to-signal-active/10 border border-accent-primary/30 flex items-center justify-center">
                <svg
                  className="w-6 h-6 text-accent-primary/60"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="1.5"
