...
export function EvolutionView({ activeProject, worldId }: EvolutionViewProps) {
  ...
  const handleGenerationSelect = useCallback(
    (gen: GenerationSummary) => {
      ...
      const node = evolutionState.mutationHistory.find((n) => n.generation === gen.generation);
      if (node) {
    ...
    <div className="h-full flex flex-col gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button className="mc-btn text-sm px-3 py-1.5">{"\u25B6"} Start Evolution</button>
          <button className="mc-btn text-sm px-3 py-1.5">{"\u23F8"} Pause</button>
        </div>
      </div>
