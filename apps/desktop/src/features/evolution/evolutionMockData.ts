...
export const MOCK_GENOME: GenomeParameter[] = [
  {
    ...
    value: "dramatic",
    type: "enum",
    options: ["natural", "dramatic", "soft", "high_contrast"],
  },
  ...
  { name: "furniture.density", value: 0.75, type: "number", min: 0, max: 1, step: 0.05 },
  {
    ...
    value: "limestone_weathered",
    type: "enum",
    options: ["marble", "granite", "limestone_weathered", "concrete"],
  },
  {
    name: "materials.wood",
    value: "oak_dark",
    type: "enum",
    options: ["oak_light", "oak_dark", "walnut", "mahogany"],
  },
  { name: "ceiling.height_m", value: 4.5, type: "number", min: 3, max: 8, step: 0.5 },
  {
    ...
    value: "arched",
    type: "enum",
    options: ["rectangular", "arched", "circular", "clerestory"],
  },
...
export function generateParetoFrontier(maxGeneration: number): ParetoPoint[] {
  ...
  for (let i = 0; i < maxGeneration * 3; i++) {
    ...
    const speed = Math.max(
      0.1,
      Math.min(0.95, baseSpeed - (quality - 0.5) * 0.3 + (rand() - 0.5) * 0.2)
    );
