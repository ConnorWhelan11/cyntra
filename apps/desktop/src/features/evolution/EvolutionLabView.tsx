...
function computeGenomeDiff(current: GenomeParameter[], parent: GenomeParameter[]): GenomeDiff {
  ...
  const similarity = current.length > 0 ? unchanged.length / current.length : 1;
...
export function EvolutionLabView({ activeProject, worldId }: EvolutionLabViewProps) {
  ...
  const candidates = useMemo(
    () => evolutionState.paretoFrontier.map((p) => paretoPointToCandidate(p, genomeParams)),
  ...
  const parentCandidate = useMemo(() => {
    ...
    return candidates.find((c) => c.generation === selectedCandidate.generation - 1) || null;
  ...
  const highlightedPath = useMemo(() => {
    ...
    while (current.parentId) {
      const parent = evolutionState.mutationHistory.find((n) => n.id === current.parentId);
      if (parent) {
        path.push(parent.id);
        current = parent;
      } else break;
  ...
  const handleMutationNodeClick = useCallback(
    (node: MutationNode) => {
  ...
  const handleParameterChange = useCallback((name: string, value: number | string) => {
    setGenomeParams((prev) => prev.map((p) => (p.name === name ? { ...p, value } : p)));
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center animate-organic-breathe">
          <div className="w-24 h-24 mx-auto mb-4 shape-organic bg-evo-cytoplasm flex items-center justify-center glow-membrane">
    ...
    <div className="h-full flex flex-col gap-3 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0 px-1">
        <div>
          <h1 className="text-xl font-semibold text-primary tracking-tight flex items-center gap-2">
            <span
              ...
              style={{
                background: runState === "running" ? "var(--evo-mitosis)" : "var(--evo-nucleus)",
                animation:
                  runState === "running" ? "nucleus-glow 1.5s ease-in-out infinite" : "none",
              }}
            />
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleStartPause} className="mc-btn text-sm px-3 py-1.5">
