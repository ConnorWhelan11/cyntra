...
export function AgentLog({ events, agentId, maxEvents = 100 }: AgentLogProps) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [autoScroll, setAutoScroll] = useState(true);

  // Filter and limit events
  const displayEvents = events.filter((e) => !e.agentId || e.agentId === agentId).slice(-maxEvents);
  ...
  return (
    <div ref={scrollRef} className="agent-log" onScroll={handleScroll}>
      {displayEvents.map((event, index) => (
        <div key={event.id || index} className={`agent-log-event ${EVENT_COLORS[event.type]}`}>
          <span className="agent-log-event-icon">{EVENT_ICONS[event.type]}</span>
          <span className="agent-log-event-time">{formatTime(event.timestamp)}</span>
          <span className="agent-log-event-message">{formatMessage(event)}</span>
