...
export async function openViewport(payload: OpenViewportPayload = {}): Promise<string> {
  ...
  await sendViewportCommand(
    createViewportCommandEnvelope({ message_type: "OpenViewport", payload }, requestId)
  );
  return requestId;
}
...
export async function closeViewport(payload: CloseViewportPayload = {}): Promise<string> {
  const requestId = uuidV4();
  await sendViewportCommand(
    createViewportCommandEnvelope({ message_type: "CloseViewport", payload }, requestId)
  );
  return requestId;
}
...
export async function shutdownViewport(payload: ShutdownPayload = {}): Promise<string> {
  const requestId = uuidV4();
  await sendViewportCommand(
    createViewportCommandEnvelope({ message_type: "Shutdown", payload }, requestId)
  );
  return requestId;
}
