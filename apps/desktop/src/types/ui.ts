...
export interface GalleryAsset {
  ...
  // Playability (for worlds with NitroGen testing)
  playabilityVerdict?: 'pass' | 'fail' | 'pending' | 'not_run';
  playabilityScore?: number;  // 0-1 overall playability score
  playabilityMetrics?: {
    stuckRatio: number;
 ...
 */
export type GalleryInspectorTab = 'asset' | 'provenance' | 'gates' | 'playability' | 'actions';
...
/**
 * Progressive preview stages
 */
export type BuildStage = 'concept' | 'geometry' | 'textured' | 'final';

export type PreviewStage = BuildStage;

export type PreviewStageMap = Partial<Record<BuildStage, string>>;
...
export interface WorldBuildState {
  ...
  previewUrls?: PreviewStageMap;    // Progressive preview URLs
  ...
  // Playtest state (NitroGen-based gameplay testing)
  playtestStatus?: 'idle' | 'running' | 'passed' | 'failed';
  playtestJobId?: string;
