  ...
  if (param.type === "number") {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-mono text-primary w-12 text-right">{current.toFixed(2)}</span>
        {pendingValue !== undefined && pendingValue !== param.value && (
...
function IntentArrow({ from, to }: { from: number | string; to: number | string }) {
  ...
  return (
    <span
      ...
      style={{
        color: isIncrease ? "var(--evo-high)" : isDecrease ? "var(--evo-low)" : "var(--evo-mid)",
      }}
    >
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-2 border-b border-slate/30 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {hasChanges && (
            <span className="text-[10px] text-warning animate-pulse">Unsaved changes</span>
          )}
          <span className="text-[10px] text-tertiary font-mono">{parameters.length} params</span>
        </div>
      </div>
      ...
      <div className="p-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-1 flex-wrap">
            {groups.map((group) => (
              ...
              >
                ...
                <span className="text-[10px] opacity-60">({group.parameters.length})</span>
              </button>
            ))}
          </div>
          ...
          {!activeGroupData && (
            <div className="text-center text-tertiary text-sm py-4">Select a parameter group</div>
          )}
        </div>
      </div>
    </div>
