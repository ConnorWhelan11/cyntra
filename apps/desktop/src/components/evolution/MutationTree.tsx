...
function layoutTree(nodes: MutationNode[], maxGen: number): Map<string, LayoutNode> {
...
export function MutationTree({
  ...
  // Layout the tree
  const layout = useMemo(() => layoutTree(history, maxGen), [history, maxGen]);
  ...
  const edges = useMemo(() => {
    ...
    for (const [id, node] of layout) {
      if (node.parentId) {
        ...
        if (parent) {
          const isHighlighted =
            highlightedPath.includes(id) && highlightedPath.includes(node.parentId);
          result.push({
    ...
    <div className={`mc-panel ${className}`}>
      <div className="mc-panel-header">
        ...
        <div className="mc-panel-actions">
          <span className="text-xs text-tertiary font-mono">{layout.size} nodes</span>
        </div>
      </div>
              ...
              return (
                <g transform={`translate(${tooltipX}, ${tooltipY})`}>
                  ...
                  <text
                    ...
                    fontFamily="var(--font-mono)"
                  >
                    Fitness: {node.fitness.toFixed(3)} ({node.fitnessChange >= 0 ? "+" : ""}
                    {node.fitnessChange.toFixed(3)})
                  </text>
                </g>
              );
            })()}
        ...
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1">
            <span
              ...
              style={{ background: mutationColors.mutation }}
            />
            Mutation
          </span>
          <span className="flex items-center gap-1">
            <span
              className="w-2 h-2 rounded-full"
              style={{ background: mutationColors.crossover }}
            />
            Crossover
          </span>
          <span className="flex items-center gap-1">
            <span
              className="w-2 h-2 rounded-full"
              style={{ background: mutationColors.selection }}
            />
            Selection
          </span>
        </div>
