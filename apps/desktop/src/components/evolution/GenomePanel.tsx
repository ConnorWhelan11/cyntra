...
export function GenomePanel({
  ...
  const handleSliderChange = (param: GenomeParameter, e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseFloat(e.target.value);
    onChange?.(param.name, value);
  };

  const handleSelectChange = (param: GenomeParameter, e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
        ...
        {parameters.map((param) => (
          <div key={param.name} className="flex items-center gap-4">
            ...
            {param.type === "string" && !param.options && (
              <span className="flex-1 text-sm text-primary font-mono">{param.value}</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 pt-2 border-t border-slate">
        {onMutateRandom && (
          <button onClick={onMutateRandom} disabled={disabled} className="mc-btn flex-1">
            Mutate Random
          </button>
        )}
        {onCrossover && (
          <button onClick={onCrossover} disabled={disabled} className="mc-btn flex-1">
            Crossover
          </button>
        )}
        {onResetToBest && (
          <button onClick={onResetToBest} disabled={disabled} className="mc-btn flex-1">
            Reset to Best
          </button>
        )}
      </div>
