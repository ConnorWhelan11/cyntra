...
export function KernelHeader({
  ...
  const glyphState = hasError ? "error" : isRunning ? "running" : "idle";
  ...
  return (
    <header className="kernel-header">
      ...
      <div className="kernel-header-actions">
        <Button variant="ghost" onClick={onRefresh} disabled={!hasProject}>
          Refresh
        </Button>
        <Button variant="ghost" onClick={onInitBeads} disabled={!hasProject}>
          Init Beads
        </Button>
        <Button variant="primary" onClick={onNewIssue} disabled={!hasProject}>
          New Issue
        </Button>
        <Button variant="ghost" onClick={onInitKernel} disabled={!hasProject}>
          Init
        </Button>
        <Button variant="ghost" onClick={onRunOnce} disabled={!hasProject}>
          Run Once
        </Button>
        {!isRunning ? (
          <Button variant="primary" onClick={onWatch} disabled={!hasProject}>
            Watch
          </Button>
        ) : (
