...
function JsonViewer({ content }: { content: string }) {
  try {
    ...
    return <pre className="artifact-viewer-json">{formatted}</pre>;
  } catch {
    return <pre className="artifact-viewer-text">{content}</pre>;
  }
}
...
function ImageViewer({ url }: { url: string }) {
  return (
    <div className="artifact-viewer-image-container">
      <img src={url} alt="Artifact preview" className="artifact-viewer-image" />
    </div>
  );
}
...
function GlbViewer({ url }: { url: string }) {
  ...
  return (
    <div className="artifact-viewer-placeholder">
      ...
      <a href={url} download className="artifact-viewer-download-link">
        Download GLB
      </a>
    </div>
  );
...
function DownloadViewer({ artifact, url }: { artifact: ArtifactNode; url: string }) {
  return (
    <div className="artifact-viewer-placeholder">
      ...
      <span className="artifact-viewer-placeholder-size">{formatBytes(artifact.sizeBytes)}</span>
      <a href={url} download className="artifact-viewer-download-link">
        Download File
      </a>
    </div>
  ...
  if (artifact.isDir) {
    return (
      <div className="artifact-viewer-empty">
        ...
        <span className="artifact-viewer-empty-meta">{artifact.children.length} items</span>
      </div>
    );
  }
