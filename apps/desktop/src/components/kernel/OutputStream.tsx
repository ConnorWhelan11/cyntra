...
export function OutputStream({
  ...
  const truncatedRunId = runId ? (runId.length > 20 ? `...${runId.slice(-16)}` : runId) : null;
    ...
    <div className={`output-stream ${isExpanded ? "expanded" : "collapsed"}`}>
      ...
      >
        ...
        <div className="output-stream-header-right">
          <span className="output-stream-toggle">{isExpanded ? "▼" : "▲"}</span>
        </div>
      </div>
            ...
            {filteredEvents.length === 0 ? (
              ...
              filteredEvents.map((event, idx) => {
                ...
                return (
                  ...
                  >
                    <span className={`output-stream-event-icon ${statusClass}`}>{icon}</span>
                    <span className="output-stream-event-type">{event.type}</span>
                    <span className="output-stream-event-meta">
                      {timestamp}
                      {event.issueId && (
                        <span className="output-stream-event-issue">#{event.issueId}</span>
                      )}
                      {event.workcellId && (
                        <span className="output-stream-event-wc">{event.workcellId.slice(-6)}</span>
                      )}
                    </span>
                  </div>
                );
