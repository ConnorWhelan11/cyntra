...
export function OutputDock({ runId, output, isRunning }: OutputDockProps) {
  ...
  const truncatedRunId = runId ? (runId.length > 32 ? `...${runId.slice(-28)}` : runId) : null;
    ...
    <div className={`output-dock ${isExpanded ? "expanded" : "collapsed"}`}>
      ...
      >
        <div className="output-dock-header-left">
          ...
          {truncatedRunId && <span className="output-dock-run-id">{truncatedRunId}</span>}
        </div>
        <div className="output-dock-header-right">
          <span className="output-dock-toggle">{isExpanded ? "collapse" : "expand"}</span>
        </div>
      </button>
