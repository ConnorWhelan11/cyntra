  ...
  return (
    <div className={`workcell-bar-item ${progressClass}`}>
      ...
      <div className="workcell-bar-status">
        {workcell.proofStatus && <StatusBadge status={workcell.proofStatus} />}
      </div>

      <div className="workcell-bar-actions">
        <button type="button" className="mc-btn-icon" onClick={onViewDetails} title="View Details">
          info
        </button>
        <button
          type="button"
...
export function WorkcellGrid({ workcells, onViewDetails, onOpenTerminal }: WorkcellGridProps) {
  return (
