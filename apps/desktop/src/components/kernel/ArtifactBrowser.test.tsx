...
describe("ArtifactBrowser", () => {
  ...
  it("shows empty state when no projectRoot provided", () => {
    render(
      <ArtifactBrowser runId="run_123" projectRoot={null} serverBaseUrl="http://localhost:3000" />
    );
  ...
  it("shows error state when fetch fails", async () => {
    vi.mocked(runService.getArtifactsTree).mockRejectedValueOnce(new Error("Network error"));
