...
export function WorldCard({ world, selected = false, onClick, onDoubleClick }: WorldCardProps) {
    ...
    >
      ...
      {isBuilding && world.progress !== undefined && (
        <div className="world-card-progress workcell-progress">
          <div className="world-card-progress-fill" style={{ width: `${world.progress}%` }} />
        </div>
      )}
      ...
      <div className="world-card-meta">
        ...
        {world.fitness !== undefined && (
          <span className="world-card-fitness">fit:{world.fitness.toFixed(2)}</span>
        )}
      </div>
    </div>
  );
